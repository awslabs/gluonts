...
class GNNInfExperiment:
    def __init__(
        ...
        self.device = device
        self.target_dim = int(self.dataset.metadata.feat_static_cat[0].cardinality)

    def run(self) -> None:
        metadata, train_dataset, test_dataset = self.dataset.metadata, self.dataset.train, self.dataset.test
        freq = metadata.freq
        prediction_length = metadata.prediction_length
        context_length = 10 * prediction_length

        # Retrieve estimator
        estimator = self.estimator(freq=freq, prediction_length=prediction_length, context_length=context_length,
                                   batch_size=self.batch_size, num_batches_per_epoch=self.num_batches_per_epoch,
                                   epochs=self.epochs, lr=self.lr, gnn_name=self.gnn_name, nf=self.nf,
                                   gnn_layers=self.gnn_layers, device=self.device)
        # Train
        ...
        forecast_it, ts_it = make_evaluation_predictions(dataset=test_dataset, predictor=predictor)
        forecasts_pytorch = list(f.to_sample_forecast() for f in forecast_it)
        tss_pytorch = list(ts_it)
        agg_metrics, item_metrics = evaluator(iter(tss_pytorch), iter(forecasts_pytorch), num_series=len(test_dataset))

        for name in ["mean_wQuantileLoss", "RMSE", "ND"]:
            print(f"{name}", agg_metrics[name])


def retrieve_multivariate_dataset(dataset_name: str, regenerate: bool=False, max_num_nodes: int=100000) -> TrainDatasets:
    ...
    grouper_test = MultivariateGrouper(max_target_dim=max_num_nodes, num_test_dates=int(num_test_dates))
...
def test_gluonts_gnninf() -> None:
    ...
    expt = GNNInfExperiment(dataset=dataset, estimator=GNNInfEstimator, device=device)
    expt.run()
...
def test_torch_gnninf() -> None:
    ...
    # Model init
    fcgnn = GNNInfNetwork(in_channels, out_channels, input_length, pred_length, num_nodes, agg_name=agg_name)
