...
def simple_add_time_features(
    ...
    if pred_length > 0:
        index = index.union(
            pd.date_range(index[-1] + index.freq, index[-1] + pred_length * index.freq, freq=index.freq)
        )
...
def test_AddTimeFeatures_correctness(
    ...
    for entry, transformed_entry in zip(dataset, transform(dataset, is_train=(pred_length == 0))):
        expected_features = simple_add_time_features(entry, time_features, pred_length)
        assert np.allclose(
            expected_features,
            transformed_entry["features"]
        )
