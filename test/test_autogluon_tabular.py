...
import pandas as pd
import numpy as np
from gluonts.nursery.autogluon_tabular import LocalTabularPredictor, get_prediction_dataframe
...
def test_autogluon_tabular():
    ...
    predictor = LocalTabularPredictor(
        freq=freq, prediction_length=prediction_length, use_lag=True, lags=[]
    )
...
def test_get_dataframe():
    ...
    prediction_length = 5
    ts = to_pandas(list(dataset)[0])
    df = get_prediction_dataframe(ts, prediction_length, [1, 2, 5], train=True)
    df[['lag1', 'lag2', 'lag5']] = df[['lag1', 'lag2', 'lag5']].apply(lambda x: round(x, 2))
    ls = list(df.loc[5, ['lag1', 'lag2', 'lag5']].values)
    assert np.array(list(i == j for i, j in zip(ls, [34096.95, 35790.55, 1089.2]))).all()
    return df
