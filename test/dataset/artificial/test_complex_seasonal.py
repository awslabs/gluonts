...
@pytest.mark.parametrize(
    ...
    zip(_length_low, _length_low + np.array([50]))
)
@pytest.mark.parametrize(
    'min_val, max_val',
    zip(_min_val, _min_val + np.array([5000, 2500, 1000]))
)
@pytest.mark.parametrize(
    'is_integer',
    [True, False]
...
def test_complex_seasonal(
        num_series: int, prediction_length: int, freq_str: str, length_low: int, length_high: int, min_val: float,
        max_val: float, is_integer: bool, proportion_missing_values: float, is_noise: bool, is_scale: bool,
        percentage_unique_timestamps: float, is_out_of_bounds_date: bool, clip_values: bool
) -> None:
    context = do_not_raise() if length_low > prediction_length else pytest.raises(AssertionError)
    with context:
        ...
        for ts_train, ts_test in zip(train, test):
            ...
            if is_integer:
                assert np.array(train_values).dtype == np.array(test_values).dtype == np.int
