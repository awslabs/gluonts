...
class WaveNetEstimator(GluonEstimator):
    """
        ...
        bin_centers
            Optional. Bin centers of the Binned distribution.
            If provided `num_bins` parameter is ignored.
        ...
        sample_based_forecast
            Flag to indicate whether the forecast generated by the predictor is sample based or not.
    """
    ...
    def __init__(
        ...
        sample_based_forecast: bool = True,
    ) -> None:
        """
        Model with Wavenet architecture and quantized target.
        ...
        if not sample_based_forecast:
            self.logger.info(
                "Forecast is not sample based. Ignoring `num_samples` parameter."
            )
        ...
        if bin_centers is not None:
            self.logger.info(
                "Ignoring `num_bins` parameter since `bin_centers` are provided."
            )
        ...
        self.sample_based_forecast = sample_based_forecast
    ...
    def train(
        ...
        **kwargs,
        ...
        if bin_centers is None:
            has_negative_data = any(
                np.any(d["target"] < 0) for d in training_data
            )
    ...
    def create_predictor(
        ...
        if self.sample_based_forecast:
            ...
            prediction_network = WaveNetDistributionPredictor(
                temperature=self.temperature,
                **self._get_wavenet_args(bin_values),
            )
        ...
        forecast_generator = (
            SampleForecastGenerator()
            if self.sample_based_forecast
            else DistributionForecastGenerator(
                BinnedOutput(
                    bin_centers=mx.nd.array(bin_values)
                )  # TODO: hybridize!
            )
        )
