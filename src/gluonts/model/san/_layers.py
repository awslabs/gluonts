...
def _torch_gather(F, data: Tensor, idx: Tensor, axis: int):
    ...
    for dim in range(data.ndim):
        if dim == axis:
            ...
            for d in range(data.ndim-dim-1):
...
class SinusoidalPositionalEmbedding(HybridBlock):
    ...
    def __init__(self, d_embed: int, **kwargs):
        ...
        if d_embed % 2 != 0:
            raise ValueError('sinusoidal embedding must have an even dimension')
        self.d_embed = d_embed
        inv_freq = .arange(0, self.d_embed, 2, dtype=pt.float)
...
class Attention(HybridBlock):
    ...
    def _compute_attn_output(self, F, score: Tensor, v: Tensor, **kwargs) -> Tensor:
        raise NotImplementedError
...
class GroupSelfAttention(Attention):
    ...
    def __init__(self,
        ...
        try:
            ...
            raise ValueError('Both d_hidden and n_head must be divisible by n_groups')
        ...
        with self.name_scope():
            self.qk_proj = nn.Conv1D(channels=d_hidden*2,
                                     kernel_size=1,
                                     groups=n_groups,
                                     use_bias=bias, 
                                     weight_initializer=init.Xavier(), 
                                     prefix='qk_proj_')
            self.v_proj = nn.Dense(units=self.d_head if self.share_values else d_hidden, 
                                   use_bias=bias, 
                                   flatten=False, 
                                   weight_initializer=init.Xavier(), 
                                   prefix='v_proj_')
            self.out_proj = nn.Dense(units=d_hidden, 
                                     use_bias=bias, 
                                     flatten=False, 
                                     weight_initializer=init.Xavier(), 
                                     prefix='out_proj_')
            ...
            if self.dist_enc is not None:
                assert self.dist_enc in ['dot', 'add'], f'distance encoding type {self.dist_enc} is not supported'
                self.posemb = SinusoidalPositionalEmbedding(d_hidden)
                self.pos_proj = nn.Dense(units=d_hidden, 
                                         use_bias=bias, 
                                         flatten=False, 
                                         weight_initializer=init.Xavier(), 
                                         prefix='pos_proj_')
                if self.apply_rel_dist == 'add':
                    self._ctt_bias_weight = Parameter('_ctt_bias_weight', shape=(1, n_head, 1, self.d_head), init=init.Xavier())
                    self._pos_bias_weight = Parameter('_pos_bias_weight', shape=(1, n_head, 1, self.d_head), init=init.Xavier())

    def _compute_qkv(self, 
                     F, 
                     value: Tensor,
                     shape: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    def _apply_mask(self, 
                    F, 
                    score: Tensor,
                    key_mask: Optional[Tensor]) -> Tensor:
        if not self.bidirectional:
            ...
            score = F.where(unidir_mask, F.ones_like(score)*float('-inf'), score)
        if key_mask is not None:
            mem_mask = key_mask.expand_dims(axis=1) # head
            mem_mask = key_mask.expand_dims(axis=2) # query
            score = F.where(mem_mask, F.ones_like(score)*float('-inf'), score)
        return score
    ...
    def _compute_attn_score(self,
                            ...
                            _pos_bias_weight: Optional[Tensor],) -> Tensor:
        score = F.batch_dot(lhs=q, rhs=k, transpose_b=True)
        if _pos_proj_weight is not None:
            # score_{ij} = <q_i, k_j> + s_{ij}
            # idx.shape = [klen, klen]
            # idx[i][j] = i-j
            idx = F.contrib.arange_like(k, axis=1)
            idx = F.broadcast_sub(idx.expand_dims(axis=1), idx.expand_dims(axis=0))
            # idx[i][j] = |i-j|
            idx = idx.abs()
            ...
            if _ctt_bias_weight is not None and _pos_bias_weight is not None:
                ...
                rk = F.batch_dot(u, k, transpose_b=True) + F.batch_dot(v, r, transpose_b=True)
    ...
    def hybrid_forward(self,
                       ...
                       *,
                       ...
                       _pos_bias_weight: Optional[Tensor] = None) -> Tensor:
        q,k,v = self._compute_qkv(F, value, shape)
        score = self._compute_attn_score(F, q, k, mask, _ctt_bias_weight, _pos_bias_weight)
...
class DualSelfAttention(GroupSelfAttention):
    ...
    def __init__(self, **kwargs):
        ...
        with self.name_scope():
            self.pat_proj = nn.Conv1D(channels=d_hidden*2,
                                      kernel_size=1,
    ...
    def hybrid_forward(self, 
                       ...
                       *,
                       mask: Optional[Tensor] = None,
                       _ctt_bias_weight: Optional[Tensor] = None,
                       _pos_bias_weight: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        q, k, v = self._compute_qkv(F, value, shape)
        score = self._compute_attn_score(F, q, k, mask, _ctt_bias_weight, _pos_bias_weight)
        v, s = self._compute_attn_output(F, score, v)
        return v, s
...
class PosFFN(HybridBlock):
    ...
    def __init__(self,
        ...
        self.pre_ln = pre_ln
        with self.name_scope():
            self.linear1 = nn.Dense(units=d_hidden,
                                    use_bias=True,
                                    flatten=False,
...
class CausalConv1D(HybridBlock):
    ...
    def hybrid_forward(self, F, x: Tensor, *args) -> Tensor:
        pad = F.zeros_like(x).slice_axis(axis=1, begin=0, end=1).tile(reps=(1,self.kernel_size-1,1))
