...
class MQF2Distribution(Distribution):
    ...
    multivariate_item
        Boolean variable
        True: Indicating multivariate across time series
        False: Indicating multivariate in prediciton time horizon
    target_dim
        When multivariate_item = True, indicate number of time series
    validate_args
    ...
    def __init__(
        ...
        self.multivariate_item = multivariate_item
    ...
    def log_prob(self, z: torch.Tensor) -> torch.Tensor:
        ...
        # Multivariate in prediction time horizon
        if self.target_dim == 1:
            z = self.stack_sliding_view(z)
        # Multivariate across item
        else:
            z = z.reshape(-1, self.target_dim)
    ...
    def rsample(self, sample_shape: torch.Size = torch.Size()) -> torch.Tensor:
        ...
        multivariate_dim = (
            self.target_dim if self.target_dim > 1 else self.prediction_length
        )

        alpha = torch.rand(
            (num_samples, multivariate_dim),
        ...
        return self.quantile(alpha, hidden_state_repeat).reshape(
            (numel_batch,) + sample_shape + (multivariate_dim,)
        )
    ...
    def get_numel(tensor_shape: torch.Size) -> int:
        ...
        return (
            torch.prod(torch.tensor(tensor_shape)).item()
            if tensor_shape
            else 1
        )
    ...
    def event_shape(self) -> Tuple:
        return (self.target_dim,) if self.multivariate_item else ()
...
class MQF2DistributionOutput(DistributionOutput):
    ...
    def distribution(
        ...
        distr = self.distr_cls(
            ...
            beta=self.beta,
            multivariate_item=self.multivariate_item,
...
class TransformedMQF2Distribution(TransformedDistribution):
    ...
    def energy_score(self, y: torch.Tensor) -> torch.Tensor:
        ...
        if self.base_dist.target_dim == 1:
            repeated_scale = self.repeat_scale(scale)
            return loss * (repeated_scale ** beta)
        else:
            # In the case of multivariate in time series,
            # we scale each entry of the observation vector differently.
            #
            # That is, in scaling, we compute the element-wise multiplication
            # between the scale vector and the observation vector
            # (with size target_dim).
            #
            # In the energy score computation, the norm is taken for the
            # observation vector, therefore we cannot scale it back after
