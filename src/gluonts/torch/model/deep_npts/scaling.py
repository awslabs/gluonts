...
def _one_if_too_small(x, min_value):
    return torch.where(
        x >= min_value, x, torch.ones(tuple(1 for _ in x.shape), dtype=x.dtype)
    )


def min_max_scaling(seq, dim=-1, keepdim=False, min_scale=1e-6):
...
def standard_normal_scaling(seq, dim=-1, keepdim=False, min_scale=1e-6):
