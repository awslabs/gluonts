...
from .module import DecoderTransformer


class LogSparseTransformerLightningModule(pl.LightningModule):
    """
    ...
    def _compute_loss(self, batch, batch_idx: int):  # type: ignore
        mu, sigma = self.model(series_id, train_seq)
        distr = torch.distributions.Normal(
            scaling_factor * mu, scaling_factor * sigma
        )
        return -distr.log_prob(gt).mean()
