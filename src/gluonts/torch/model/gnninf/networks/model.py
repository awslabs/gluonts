...
class GNNInfNetwork(torch.nn.Module):
    ...
    :param in_channels: number of channels/dimensions in the input sequence
    (this value is usually set to 1)
    :param out_channels: number of channels/dimensions in the output sequence
    (this value is usually set to 1)
    ...
    :param use_unique_id: Use unique identifier for the nodes,
    we recommend setting it True
    :param enc_name: encoder network, it can be "mlpres" or "cnnres"
    :param dec_name: decoder network, it can only be "mlpres"
    :param agg_name: aggregation type 'gnn' (FCGNN) or 'bpgnn' for the (BPGNN)
    ...
    :param enc_layers: number of layers in the encoder when of using "mlpres"
    :param dec_layers: number of layers in the decoder
    :param edge_nf_ratio: the number of features in edges is divided by
    nf/edge_nf_ratio
    ...
    def __init__(
        ...
        if self.agg_name == "gnn":
            self.gnn = gnns.FC_GNN(
                self.nf_enc + self.id_embedding_size,
                nf,
                nf,
                n_layers=gnn_layers,
                residual=residual,
            ...
            self.gnn = gnns.BP_asynchron_GNN(
                n_latent_nodes,
                id_embedding_size,
                True,
                self.nf_enc + self.id_embedding_size,
        ...
        self.unique_id = nn.Parameter(
            torch.randn(1, self.num_nodes, self.id_embedding_size)
        )

    def forward(
        self, inputs: torch.Tensor, retrieve_edges: bool = False
    ) -> torch.Tensor:
        """
        ...
        if self.use_unique_id:
            unique_id = self.unique_id.repeat(bs, 1, 1).view(
                bs * num_sensor, -1
            )
        ...
        outputs = outputs.view(
            bs, num_sensor, self.pred_length, self.out_channels
        )
    ...
    def encode(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.enc_name == "mlpres" or self.enc_name == "linear":
            ...
            inputs = inputs.reshape(
                bs * num_sensor, seq_len, input_dim
            ).transpose(1, 2)
