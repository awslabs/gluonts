...
class GNN(nn.Module):
    def __init__(self, in_node_nf, hidden_nf, out_node_nf, in_edge_nf=0, device='cpu', act_fn=nn.SiLU(), n_layers=4, residual=True, attention=False, del_edges=False, C=1.0, edge_nf_ratio=2, clamp=False):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, GCL(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf,
                                              act_fn=act_fn, residual=residual, attention=attention, del_edges=del_edges,
                                            edge_nf_ratio=edge_nf_ratio, clamp=self.clamp))
        self.to(self.device)
    ...
    def forward(self, h, edges, edge_attr=None, retrieve_edges=False):
        ...
        for i in range(0, self.n_layers):
            h, att_val = self._modules["gcl_%d" % i](h, edges, edge_attr=edge_attr, C=self.C)
            if retrieve_edges:
                inf_edges.append({"edge_indexes": edges, "edge_values": att_val})
...
class FC_GNN(nn.Module):
    ...
    def forward(self, h, retrieve_edges=False):
        ...
        return self.gnn(h, edges, edge_attr=None, retrieve_edges=retrieve_edges)
...
class AsynchronousGNN(GNN):
    ...
    def forward(self, h, edges_a, edges_b, update_mask_a, update_mask_b, retrieve_edges=False):
        inf_edges = []
        h = self.embedding_in(h)
        for i in range(0, self.n_layers):
            if i % 2 == 0:
            ...
            if self.different_mlps == False:
                i = int(i/2)
            h, att_val = self._modules["gcl_%d" % i](h, edges, update_mask=update_mask, C=C)
            if retrieve_edges:
                inf_edges.append({"edge_indexes": edges, "edge_values": att_val})
        h = self.embedding_out(h)
        return h, inf_edges
...
class BP_asynchron_GNN(nn.Module):
    def __init__(self, num_latent_nodes, id_embedding_size, different_mlps, *args, **kwargs):
        super(BP_asynchron_GNN, self).__init__()
        self.gnn = AsynchronousGNN(num_latent_nodes, different_mlps, *args, **kwargs)
        self.to(self.gnn.device)
        self.num_latent_nodes = num_latent_nodes
        self.edges_dict = {}
        self.id_latent = nn.Parameter(torch.randn(1, self.num_latent_nodes, id_embedding_size))
    ...
    def forward(self, h, retrieve_edges=False):
        ...
        edges_a, edges_b = self.get_edges(num_sensor, self.num_latent_nodes, bs)
        ...
        update_mask_a, update_mask_b = self.get_masks(bs, num_sensor, self.num_latent_nodes)
        #Reshape to fit into the GNN
        h = h.view(bs * (num_sensor + self.num_latent_nodes), -1)

        h, inf_edges = self.gnn(h, edges_a, edges_b, update_mask_a, update_mask_b, retrieve_edges=retrieve_edges)
        h = h.view(bs, num_sensor + self.num_latent_nodes, -1)
    ...
    def get_masks(self, bs, n_nodes, n_latent_nodes):
        ...
        mask_b = 1 - mask_a
        return mask_a.to(self.gnn.device), mask_b.to(self.gnn.device),
    ...
    def get_edges(self, n_nodes, num_latent_nodes, bs):
        ...
        if bs not in self.edges_dict[key]:
            ...
            edges_a, _ = cast_edges_batch(n_nodes + num_latent_nodes, bs, edges_a)
            edges_b, _ = cast_edges_batch(n_nodes + num_latent_nodes, bs, edges_b)
            self.edges_dict[key][bs] = [edges_a, edges_b]

        [[rows_a, cols_a], [rows_b, cols_b]] = self.edges_dict[key][bs]
        return [[rows_a.to(self.gnn.device), cols_a.to(self.gnn.device)], [rows_b.to(self.gnn.device), cols_b.to(self.gnn.device)]]
...
def cast_edges_batch(n_nodes, batch_size, edges=None):
    if edges is None:
        edges = get_fc_edges(n_nodes)  # If no edges are provided load fully connected edges
