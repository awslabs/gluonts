...
class DeepVARModel(DeepARModel):
    @validated()
    def __init__(
        self,
        freq: str,
...
class VecLaggedLSTM(LaggedLSTM):
    def __init__(
        self,
        input_size: int,
        features_size: int,
        ...
        self.target_dim = target_dim
        self.rnn_input_size = (
            input_size * len(self.lags_seq) * self.target_dim + features_size
        )
        self.rnn = nn.LSTM(
    ...
    def get_lagged_subsequences(
        ...
        for lag_index in indices:
            ...
            lagged_values.append(
                sequence[:, begin_index:end_index, ...].unsqueeze(dim=1)
            )
    ...
    def _check_shapes(
        ...
        assert len(prior_input.shape) == 3 and self.input_size == 1
        assert len(input.shape) == 3 and self.input_size == 1
        assert (
