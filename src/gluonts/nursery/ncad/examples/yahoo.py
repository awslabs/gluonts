...
def yahoo_inject_anomalies(
        ...
        rate_true_anomalies_used: float = 1.,
    ...
    ts_transform = tr.LabelNoise( p_flip_1_to_0=1.-rate_true_anomalies_used ) # Ignore some true labels
    ...
    if injection_method == 'None':
        ...
        dataset_transformed = ncad.utils.take_n_cycle( ts_transform_iterator, multiplier * len(dataset))
        dataset_transformed = TimeSeriesDataset(dataset_transformed)
    else:
...
def yahoo_pipeline(
    ...
    data_dir, model_dir, log_dir = [ PosixPath(path).expanduser() if str(path).startswith('~') else Path(path) for path in dirs ]
    ...
    base_dataset = TimeSeriesDataset(ncad.utils.take_n_cycle( scaler(base_dataset), len(base_dataset)))
    ...
    train_set, validation_set, test_set = ncad.ts.split_train_val_test(
        ...
        split_warmup_length = window_length-suspect_window_length if train_split_method == 'past_future_with_warmup' else None,
        verbose=False,
    )
    ...
    ckpt_file = [file for file in os.listdir(model_dir) if (file.endswith(".ckpt") and file.startswith('ncad-model-'+exp_name))][-1]
