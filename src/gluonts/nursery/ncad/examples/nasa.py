...
def nasa_pipeline(
    ...
    for i, ts in enumerate(train_set):
        if ts.shape[0] < window_length:
            ...
            assert pad_len>0
            warnings.warn(f"Paddign series {i} with {pad_len} timesteps on the left ({np.round(100*pad_len/ts.shape[0],0)} % of its original length)")
            ts_pad = TimeSeries(values=np.zeros((pad_len,ts.shape[1])).squeeze(),labels=np.zeros(pad_len))
