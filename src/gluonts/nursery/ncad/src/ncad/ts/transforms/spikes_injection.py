...
class LocalOutlier(TimeSeriesTransform):
    ...
    def transform(
        ...
        spike_multiplier = np.random.rand(num_spikes, ts_channels) * (self.spike_multiplier_range[1] - self.spike_multiplier_range[0]) + self.spike_multiplier_range[0]
        # random sign
        ...
        assert spike_location.shape == (num_spikes,), "The length of 'spike_location' must be equal to num_spikes"
        ...
        if ts_channels > 3:
            indices = np.random.choice(np.arange(spike_addition.size), replace=False,
                                       size=int(spike_addition.size * 0.35))
...
class SpikeSmoothed(TimeSeriesTransform):
    ...
    def transform(self, ts: TimeSeries, spike_location: Optional[int] = None) -> TimeSeries:
        ...
        spike_multiplier = (np.random.rand() + self.multiplier_range[0]) * (self.multiplier_range[1] - self.multiplier_range[0])
        ...
        spike_multiplier = spike_multiplier if np.random.choice(self.direction_options) == "increase" else - spike_multiplier
