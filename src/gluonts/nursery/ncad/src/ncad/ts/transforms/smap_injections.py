...
class SMAPSpikes(TimeSeriesTransform):
    ...
    def transform(
        ...
        if np.nanstd(values) < self.max_std:
            ...
            # this is integers
            indices_of_swap = np.random.choice(range(len(values)), int(len(values) * (15.0 / 1000.0)), False)
...
class SMAPChangePoint(TimeSeriesTransform):
    ...
    def transform(
        ...
        if np.nanstd(values) < self.max_std:
            ...
            change_location = np.random.randint(len(values)//5, len(values) - self.number_anomalous_points_after_change)
            ...
            labels_addition = np.concatenate([np.zeros(LABEL_DELAY), np.array(labels_addition[:-LABEL_DELAY])])

            values = normalise_values(values + mask)
            labels = np.logical_or(labels, labels_addition).astype(int)

            indicator[change_location:change_location+self.number_anomalous_points_after_change] = 1
