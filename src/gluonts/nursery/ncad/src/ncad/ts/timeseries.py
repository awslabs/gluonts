...
class TimeSeries:
    ...
    def to_json(self, path: Union[PosixPath,str]):
        ...
        with open( path, 'w') as f:
            json.dump( dict(values=self.values.tolist(),
...
def split_train_val_test(
        ...
        split_method: str = ['random_series','sequential','past_future','past_future_with_warmup'][0],
        split_warmup_length: Optional[int] = None,
        verbose: bool = True,
        *args, **kwargs) -> Tuple[TimeSeriesDataset, TimeSeriesDataset, TimeSeriesDataset]:
    ...
    if split_method == 'random_series':        
        ...
        train_set, val_set = sklearn.model_selection.train_test_split( train_set, test_size=val_portion/(1-test_portion) )
        ...
        if split_method == 'past_future_with_warmup':
            assert split_warmup_length is not None, "'split_warmup_length' must be given when using split_method='past_future_with_warmup'."
...
def ts_split(
    ...
    if not isinstance(indices_or_sections[0], Sequence):
        ...
        predicted_scores_split = np.split( ts.predicted_scores , indices_or_sections) if ts.predicted_scores else None
    else:
        ...
        predicted_scores_split = _overlapping_split( ts.predicted_scores , indices_or_sections) if ts.predicted_scores else None

    out = TimeSeriesDataset(
        [
        TimeSeries( values=values_split[i],
...
def ts_to_array(
    ...
    """
    out = np.concatenate(
        ( ts.values.reshape(ts.shape), ts.labels.reshape(ts.shape) ),
        axis=1 )
    return out
