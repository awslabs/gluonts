...
def swat(
    ...
    assert train_df.shape == (496800, 53), f"Problem with 'SWaT_Dataset_Normal_v0.csv', expected shape (449919, 53), loaded shape {train_df.shape}"
    assert test_df.shape == (449919, 53), f"Problem with 'SWaT_Dataset_Attack_v0.csv', expected shape (449919, 53), loaded shape {test_df.shape}"
    ...
    assert all(train_df['Label']==0), "No anomalies were expected in 'SWaT_Dataset_Normal_v0.csv', but some where found"
