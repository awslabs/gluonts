...
def get_network_instances(config):
    ...
    if (
        ...
        posterior_mlp = nn.Sequential(
            nn.Linear(
                posterior_mlp_in_dim, config["inference"]["mlp_hiddendim"]
            ),
            nn.ReLU(True),
        ...
        x_ctrl_dim = (
            config["control"]["feat_dim"] if config["control"]["x"] else 0
        )
        ...
        discrete_in_dim += (
            config["obs_dim"]
            if config["discrete_transition"]["takes_y"]
            else 0
        )
        z_ctrl_dim = (
            config["control"]["feat_dim"] if config["control"]["z"] else 0
        )
