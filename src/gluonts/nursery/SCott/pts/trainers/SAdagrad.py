...
class SAdagrad:
    ...
    def __call__(
        ...
        group_ratio = data_loaders['group_ratio'] if self.weighted_batch else None
        avg_epoch_grad = 0.0
        v_0_norm = 0.
        v_t_norm = 0.
        for epoch_no in range(self.epochs):
            ...
            for batch_no in range(self.num_batches_per_epoch):
                ...
                if iter_n == 0 or v_t_norm <= self.gamma*v_0_norm or iter_n % self.freq == 0:
                    ...
                    with timer("gradient oracle"):
                        ...
                        loss = self.inference(anchor_model, inputs, weighted_batch=self.weighted_batch, group_ratio=group_ratio)
                        loss.backward()
                ...
                loss.backward()
                with timer("gradient oracle"):
                    for p1, p2, p3 in zip(net.parameters(), sg_model.parameters(), anchor_model.parameters()):
                        if p1.grad is None or p2.grad is None or p3.grad is None:
                            continue
            ...
            num_iters = self.num_batches_per_epoch*(epoch_no+1)*2*self.batch_size+ \
                self.num_batches_per_epoch/self.freq*self.num_strata*(epoch_no+1)*self.batch_size
            avg_epoch_grad = (avg_epoch_grad*epoch_no+epoch_grad)/(epoch_no+1)
            time_in_ms = timer.totals["gradient oracle"]*1000
            writer.add_scalar('gradnorm/iters', avg_epoch_grad, (epoch_no+1)*self.num_batches_per_epoch)
            writer.add_scalar('gradnorm/grads', avg_epoch_grad, num_iters)
            writer.add_scalar('gradnorm/time', avg_epoch_grad, time_in_ms)
            writer.add_scalar('train_loss/iters', avg_epoch_loss, (epoch_no+1)*self.num_batches_per_epoch)
            writer.add_scalar('train_loss/grads', avg_epoch_loss, num_iters)
            writer.add_scalar('train_loss/time', avg_epoch_loss, time_in_ms)
            if self.eval_model and epoch_no % self.validation_freq == 0:
                writer.add_scalar('val_loss/iters', validation_loss, (epoch_no+1)*self.num_batches_per_epoch)
                writer.add_scalar('val_loss/grads', validation_loss, num_iters)
                writer.add_scalar('val_loss/time',validation_loss, time_in_ms)
                print('\nTraining Loss: {:.4f}, Test Loss: {:.4f}\n'.format(avg_epoch_loss, validation_loss))
            else:
                print('\nTraining Loss: {:.4f} \n'.format(avg_epoch_loss))
            print('Epoch ', epoch_no, ' is done!')

        writer.close()
        print('task: '+ self.task_name +' on SAdagrad with lr='+str(self.learning_rate)+' is done!')
