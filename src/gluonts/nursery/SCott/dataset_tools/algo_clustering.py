...
def KMeans_inside_dataset(num_ts_=1, num_groups=16, context_length=72, prediction_length=24, file_name='default'):
    ...
    for i in range(num_ts):
        ...
        for ts_sample_start in range(0, len(target)-len_sample, prediction_length):
            ts_slice = target[ts_sample_start:ts_sample_start+len_sample]
            feature = torch.cat((feature, torch.Tensor([ts_slice.mean(), ts_slice.var(), index % 7, index // 90])))
            index += 1
...
def KMeans_m5_dataset(num_ts_=1, num_groups=16, context_length=72, prediction_length=24, file_name='default'):
