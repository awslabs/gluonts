...
def get_m4_by_freq(context_length=72, prediction_length=24, len_per_ts=200, num_ts=50, num_groups=6, file_name='m4_freq'):
    dataset_group = [[] for i in range(num_groups)]
    whole_data = []
    ret = dict()
    datasets_name = ['m4_hourly', 'm4_daily', 'm4_weekly', 'm4_monthly', 'm4_quarterly', 'm4_yearly']
    hours_factor = [1, 24, 24*7, 24*7*30, 24*7*30*3, 24*7*30*3*4]
    for i in range(num_groups):
        ...
        for j in range(num_ts):
            ...
            for ts_sample_start in range(0, len_per_ts-len_sample, prediction_length):
...
def get_temperature_data(context_length=24, prediction_length=4, samples_per_ts=2000, num_groups=8):
    ts_file = pd.read_csv('temperature.csv')
    city_names = ['Vancouver', 'Los Angeles', 'Las Vegas', 'San Diego', 'Philadelphia', 'Montreal', 'Boston', 'Haifa']
    ...
    for gid in range(num_groups):
        ...
        while True:
            num_samples += 1
            index += 1
            ts_slice = torch.tensor(ts[index:index+context_length+prediction_length].values)
            nu = 1 + sum(ts_slice)/len(ts_slice)
            ts_slice /= nu
            if torch.sum(torch.isnan(ts_slice)).item() == 0:
                dataset_group[gid].append(
                    {'target': ts_slice, 'start': pd.Timestamp(datetime[index])}
                )
                whole_data.append(
                    {'target': ts_slice, 'start': pd.Timestamp(datetime[index])}
                )
...
def get_group_data_by_var(name, num_groups, len_sample=9):
    ...
    for i in range(num_ts):
        ...
        unsplit_start = train_entry['start']
        whole_data.append(
            {'target': unsplit_ts, 'start': unsplit_start}
        )
        for ts_sample_start in range(len(unsplit_ts)-len_sample):
            group_id = 0
            print(torch.var(torch.FloatTensor(unsplit_ts[ts_sample_start:ts_sample_start+len_sample])))
            continue
            dataset_group[group_id].append(
                {'target': unsplit_ts[ts_sample_start:ts_sample_start+len_sample], 'start': unsplit_start}
            )
...
def get_synthetic_data(model_name=None, num_groups=8, mean_boundary=1):
    ...
    for gid in range(num_groups):
        ...
        for num_ts in range(num_time_steps):
            ts_slice = torch.Tensor(ts[0][-context_length:]).view(1, context_length)
            prediction = net.get_distr(ts_slice).sample((5000,))
            prediction = sum(prediction)/len(prediction)
            ts = torch.cat([ts, prediction], dim=1)
        whole_data_list.append({'target': ts.view(len(ts[0]),)[context_length:], 'start': start})
        dataset_group.append(
            ListDataset(
                [{'target': ts.view(len(ts[0]),)[context_length:], 'start': start}],
                freq='1H'
            )
        )
    dataset = ListDataset(
...
def get_synthetic_data_linear(context_length=24, prediction_length=8, num_groups=8, steps_per_ts=1, num_duplicates=16):
    assert num_groups > 1
    freq = '1H'
    len_sample = context_length + prediction_length
        ...
        for t_step in range(2*steps_per_ts):
            while True:
                with torch.no_grad():
                    ...
                    if torch.norm(prediction) < prediction_length: #and prediction_length*0.1 < torch.norm(prediction):
            ...
            for j in range(num_duplicates):
                ...
                if t_step <= steps_per_ts:
                    pattern_group1.append({'target': ts_sample, 'start': start})
                else:
                    pattern_group2.append({'target': ts_sample, 'start': start})
...
def get_synthetic_data_linear_simple(model_name=None,
    num_groups=8,
    mean_boundary=1,
    num_duplicates=16):
    assert num_groups > 1
    ...
    for gid in range(num_groups):
        ...
        pattern_group = []
        ts = (gid+1)*mean_boundary*torch.FloatTensor(base).view(1, num_time_steps)
        for j in range(num_duplicates):
            ts += torch.normal(0, 0.01, size=ts.shape)
            whole_data_list.append({'target': ts.view(len(ts[0]),), 'start': start})
            pattern_group.append({'target': ts.view(len(ts[0]),), 'start': start})
        dataset_group.append(
            ListDataset(
                pattern_group,
                freq='1D'
...
def get_synthetic_data_sin(model_name=None,
    ...
    for gid in range(num_groups):
        ...
        pattern_group = []
        ts = (gid+1)*torch.sin(torch.FloatTensor(base)).view(1, num_time_steps)
        ts += torch.FloatTensor((gid+1)*base).view(1, num_time_steps)
        no_duplicate_whole_data_list.append(
            {'target': ts.view(len(ts[0]),), 'start': start}
        )
        for j in range(num_duplicates):
    ...
    no_duplicate_dataset = ListDataset(
        no_duplicate_whole_data_list,
        freq='1D'
    )
