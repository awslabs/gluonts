        ...
        for gid in range(pattern_number):
            for j in range(num_duplicates):
                ...
                for i in range(1, pattern_number):
                    context[unit_length*(i-1):unit_length*i] = _get_mixed_pattern(
                        context[unit_length*(i-1):unit_length*i]-unit_length*(i-1),
                        pattern[(gid+i) % pattern_number])
                ts_sample = torch.cat([
                    context,
                    _get_mixed_pattern(torch.arange(prediction_length, dtype=torch.float), pattern[gid])
                ])
                whole_data.append({'target': ts_sample, 'start': start})
                if j % 5 == 0:
                    val_data.append({'target': ts_sample+torch.normal(0, 1, ts_sample.shape), 'start':start})
                dataset_group[m*4+gid].append({'target': ts_sample, 'start': start})
    print("Generating the synthetic training data, the total number of training examples:", len(whole_data))
    ret['group_ratio'] = [len(i)/len(whole_data) for i in dataset_group]
    random.shuffle(whole_data)
    group_data = []
    ret['whole_data'] = ListDataset(whole_data,
                                    freq=freq)
    ret['val_data'] = ListDataset(val_data,
                                freq=freq)
    for group in dataset_group:
        random.shuffle(group)
        group_data.append(ListDataset(group,
                        freq=freq))
...
def group_electricity_cv(num_ts=10,
                        num_groups=14,
                        context_length=72,
    ...
    for i in range(num_ts):
        ...
        for ts_sample_start in range(0, len(unsplit_ts)-len_sample, prediction_length):
            ...
            train_full_data.append(
                {'target': ts_slice, 'start': t, 'feat_static_cat': np.array([gid])}
            )
    ...
    print("Generating the electricity training data, the total number of training examples:", len(train_full_data))
    ...
    ret['val_data'] = ListDataset(test_full_data,
                        freq=dataset.metadata.freq)
    group_data_list = []
    for group in dataset_group:
        random.shuffle(group)
    ...
    for group in dataset_group:
        random.shuffle(group)
        group_data_list.append(ListDataset(group,
                                    freq=dataset.metadata.freq))
    ret['group_data'] = group_data_list
    os.makedirs('./dataset', exist_ok=True)
    with open('./dataset/'+file_name+'.csv', 'wb') as output:
...
def group_exchangerate_cv(num_ts=10,
                        num_groups=14,
                        context_length=15,
    ...
    print("Generating the exchange rate training data, the total number of training examples:", len(train_full_data))
    ret['group_ratio'] = [len(i)/len(train_full_data) for i in dataset_group]
    random.shuffle(train_full_data)
    ret['whole_data'] = ListDataset(train_full_data,
                                    freq=dataset.metadata.freq)
...
def group_traffic_cv(num_ts=10,
                    num_groups=14,
                    context_length=72,
                    prediction_length=12,
                    file_name='default'):
    ...
    print("Generating the traffic training data, the total number of training examples:", len(train_full_data))
    ret['group_ratio'] = [len(i)/len(train_full_data) for i in dataset_group]
    random.shuffle(train_full_data)
    ret['whole_data'] = ListDataset(train_full_data,
                            freq=dataset.metadata.freq)
...
print("Finished the preprocessing data, please verify ./dataset/ contains four .csv files")