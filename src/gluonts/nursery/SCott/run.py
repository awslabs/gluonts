...
parser = argparse.ArgumentParser(description="Experiment on SCott and baselines")
...
parser.add_argument('--decreasing_step_size',
                    default=False,
                    action='store_true',
                    help='enable decreasing_step_size (default: False)')
parser.add_argument('--anchor_freq',
                    type=int,
                    default=20,
...
parser.add_argument('--num_duplicates',
                    type=int,
                    default=1, 
                    help='number of duplicates to use (for synthetic data experiment)')
parser.add_argument('--num_strata',
                    type=int,
                    default=8, 
...
if args.optimizer == "SGD":
    trainer=SGD(epochs=args.epochs,
...
if args.optimizer == "Adam":
    trainer=Adam(epochs=args.epochs,
                task_name=args.task_name,
                num_batches_per_epoch=args.num_batches_per_epoch,
                batch_size=args.batch_size,
                learning_rate=args.lr,
...
if args.optimizer == "Adagrad":
    trainer=Adagrad(epochs=args.epochs,
                task_name=args.task_name,
                num_batches_per_epoch=args.num_batches_per_epoch,
                batch_size=args.batch_size,
                learning_rate=args.lr,
...
if args.optimizer == "SCott":
    trainer=SCott(epochs=args.epochs,
                    task_name=args.task_name,
                    num_batches_per_epoch=args.num_batches_per_epoch,
                    batch_size=args.batch_size,
...
if args.optimizer == "SAdam":
    trainer=SAdam(epochs=args.epochs,
                    task_name=args.task_name,
                    num_batches_per_epoch=args.num_batches_per_epoch,
                    batch_size=args.batch_size,
                    num_strata=args.num_strata,
...
if args.optimizer == "SAdagrad":
    trainer=SAdagrad(epochs=args.epochs,
                    task_name=args.task_name,
                    num_batches_per_epoch=args.num_batches_per_epoch,
                    batch_size=args.batch_size,
                    num_strata=args.num_strata,
...
if args.optimizer == "SCSG":
    trainer=SCSG(epochs=args.epochs,
                    task_name=args.task_name,
                    num_batches_per_epoch=args.num_batches_per_epoch,
                    batch_size=args.batch_size,
                    num_strata=args.num_strata,
...
if args.model == 'lstm':
    estimator = LSTMEstimator(freq=freq,
                            prediction_length=args.prediction_length,
                            context_length=args.context_length,
...
if args.model == 'linear':
    estimator = AREstimator(freq=freq,
                            prediction_length=args.prediction_length,
                            context_length=args.context_length,
                            trainer=trainer)
