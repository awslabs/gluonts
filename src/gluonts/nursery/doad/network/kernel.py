...
class AttentionKernel(Attention):
    ...
    def __init__(self,
        ...
        for layer in range(n_enc_layer+1):
            ...
            if self.bias:
                ...
                self._key_biases.append(
                    _query_bias if self.symmetric else nn.Parameter(Tensor(d_hidden)),
                )
            else:
        ...
        for layer in range(n_dec_layer):
            ...
            d_output = self.d_hidden if layer == n_dec_layer-1 else self.d_interm
            self._output_weights.append(nn.Parameter(Tensor(d_output, d_input)))
            self._output_biases.append(nn.Parameter(Tensor(d_output)) if self.bias else None)
        ...
        if norm == 'none':
            ...
            self.attn_norm = nn.LayerNorm(self.d_hidden, elementwise_affine=False)
            self.dec_norm = nn.LayerNorm(self.d_hidden, elementwise_affine=False)
        elif norm == 'instance':
        ...
        self.register_buffer('_inter_dist_comp', pt.zeros(self.d_head), persistent=False)
        self.register_buffer('_extra_dist_comp', pt.zeros(self.d_head), persistent=False)
    ...
    def _compute_attn_score(self,
        ...
        for layer in range(self.n_enc_layer):
            q = F.linear(q, self._query_weights[layer], self._query_biases[layer])
            k = F.linear(k, self._key_weights[layer], self._key_biases[layer])
            q = F.relu(q)
            k = F.relu(k)
        q = F.linear(q, self._query_weights[self.n_enc_layer], self._query_biases[self.n_enc_layer])
        k = F.linear(k, self._key_weights[self.n_enc_layer], self._key_biases[self.n_enc_layer])
        self._query = q
        self._key = k
        ...
        if self.add_dist:
            ...
            p = pt.arange(seq_len, dtype=shape.dtype, device=shape.device).view(1,-1)
        ...
        _inter_mask = pt.eye(score.size(-1), dtype=pt.bool, device=score.device)
        ...
        _extra_score = score[..., left_pad+1:-right_pad, left_pad:-(right_pad+1)]
        _extra_mask = pt.ones_like(_extra_score[0,0], dtype=pt.bool).triu(diagonal=1)
        ...
        if self.add_dist:
            ...
            p = p[..., left_pad+1:-right_pad, left_pad:-(right_pad+1), :]
    ...
    def _compute_attn_output(self,
        ...
        hidden = output
        for layer in range(self.n_dec_layer):
            hidden = F.linear(hidden, self._output_weights[layer], self._output_biases[layer])
    ...
    def forward(self,
        ...
        (
            inter_score, extra_score,
            inter_dist_comp, extra_dist_comp,
        ) = self._compute_attn_score(
            shape, left_pad, right_pad, mask
        )
        interp = self._compute_attn_output(inter_score, inter_value, inter_dist_comp)
        extrap = self._compute_attn_output(extra_score, extra_value, extra_dist_comp)
