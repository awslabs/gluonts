...
from typing import (List, Tuple, Dict, Union,\
    Sequence, Optional, Callable, overload)
...
def _dict_equal(
        dict1: Dict,
        dict2: Dict,
        cmp_func: Callable = eq) -> bool:
    if dict1.keys() == dict2.keys():
        return all(cmp_func(dict1[k], dict2[k]) for k in dict1.keys())
    return False


def _pad_array_along_axis(array:ndarray,
        pad: Tuple[int, int],
        axis: int = 0,
        constant_values=np.nan) -> ndarray:
    full_pad = [(0,0)]*array.ndim
    full_pad[axis] = pad
    return np.pad(array, full_pad, mode='constant', constant_values=constant_values)
...
class TimeSeriesInstant(object):
    ...
    def add_features(self, 
        ...
        if data.ndim != valid_ndim:
            raise ValueError(f'{feature_type} and {data_type} type feature requires {valid_ndim}-d data')
    ...
    def categorical_features(self) -> Dict:
        return {name: self._features[name] for name in self._categorical_features}

    @property
    def numerical_features(self) -> Dict:
        return {name: self._features[name] for name in self._numerical_features}
    ...
    def __eq__(self, other) -> bool:
        return all([
            ...
            _dict_equal(self.categorical_features, other.categorical_features, np.array_equal),
            _dict_equal(self.numerical_features, other.numerical_features, np.array_equal),
        ])
...
class TimeSeries(Sequence[TimeSeriesInstant]):
    def __init__(self,
        ...
        if time_index is None:
            assert not((start_time is None) or (time_unit is None)), \
                'if `time_index` is not given, both `start_time` and `time_unit` must be specified'
            self.time_index = pd.date_range(start=start_time, periods=len(self), freq=time_unit)
        ...
        if target_names is None:
            ...
            if self.d_data > 1:
                self.target_names = [f'{target_names}_{i+1}' for i in range(self.d_data)]
            else:
                self.target_names = [target_names]
        else:
            ...
            if len(target_names) < self.d_data:
                raise ValueError(f'Not enough target names are given, {err_msg}')
            self.target_names = target_names
    ...
    def add_features(self, 
        ...
        if feature_type not in valid_feature_type:
            raise ValueError(f'feature_type should be one of {valid_feature_type}')
    ...
    def static_categorical_features(self) -> Dict:
        return {name: self._features[name] for name in self._static_features & self._categorical_features}

    @property
    def static_numerical_features(self) -> Dict:
        return {name: self._features[name] for name in self._static_features & self._numerical_features} 

    @property
    def revealed_categorical_features(self) -> Dict:
        return {name: self._features[name] for name in self._revealed_features & self._categorical_features}

    @property
    def revealed_numerical_features(self) -> Dict:
        return {name: self._features[name] for name in self._revealed_features & self._numerical_features} 

    @property
    def observed_categorical_features(self) -> Dict:
        return {name: self._features[name] for name in self._observed_features & self._categorical_features}

    @property
    def observed_numerical_features(self) -> Dict:
        return {name: self._features[name] for name in self._observed_features & self._numerical_features}         
    ...
    def dynamic_categorical_features(self) -> Dict:
        return {name: self._features[name] for name in self._dynamic_features & self._categorical_features}

    @property
    def dynamic_numerical_features(self) -> Dict:
        return {name: self._features[name] for name in self._dynamic_features & self._numerical_features}
    ...
    def __eq__(self, other):
        return all([
            ...
            self.series_name == other.series_name,
            np.array_equal(self.target_names, other.target_names),
            _dict_equal(self.static_categorical_features, other.static_categorical_features, np.array_equal),
            _dict_equal(self.static_numerical_features, other.static_numerical_features, np.array_equal),
            _dict_equal(self.revealed_categorical_features, other.revealed_categorical_features, np.array_equal),
            _dict_equal(self.revealed_numerical_features, other.revealed_numerical_features, np.array_equal),
            _dict_equal(self.observed_categorical_features, other.observed_categorical_features, np.array_equal),
            _dict_equal(self.observed_numerical_features, other.observed_numerical_features, np.array_equal),
        ])
    ...
    def index_by_timestamp(self, index):
        return pd.Series(index=self.time_index, data=np.arange(len(self.time_index))).loc[index]
        ...
        if isinstance(index, int):
            ...
            if left_pad is not None:
                if self.time_unit is not None:
                    ...
                    if len(time_index) < 2:
                        raise IndexError('Cannot pad time index with a single timestamp')
                    td = time_index[1] - time_index[0]
                time_index = np.r_[np.arange(time_index[0]-td*left_pad, time_index[0], td), time_index]
                target = _pad_array_along_axis(target, (left_pad, 0), constant_values=np.nan)
            if right_pad is not None:
                ...
                time_index = np.r_[time_index, np.arange(time_index[-1]+td, time_index[-1]+td*(right_pad+1), td)]
                target = _pad_array_along_axis(target, (0, right_pad), constant_values=np.nan)
            obj = TimeSeries(
                target,
            ...
            for name in self._static_features:
                data_type = 'cat' if name in self._categorical_features else 'num'
                data = self._features[name]
                obj.add_features(name, data, data_type=data_type, feature_type='static')
            for name in self._revealed_features | self._observed_features:
                data_type = 'cat' if name in self._categorical_features else 'num'
                feature_type = 'revealed' if name in self._revealed_features else 'observed'
                val = -1 if data_type=='cat' else np.nan
                data = self._features[name][index]
                if left_pad is not None:
                    data = _pad_array_along_axis(data, (left_pad, 0), constant_values=val)
                if right_pad is not None:
                    data = _pad_array_along_axis(data, (0, right_pad), constant_values=val)
                obj.add_features(name, data, data_type=data_type, feature_type=feature_type)
            return obj
    ...
    @classmethod
    def from_pandas_series(cls, series: pd.Series, target_name: Optional[str] = None):
        return cls(
            series.values, 
    ...
    def to_pandas_dataframe(self) ->  pd.DataFrame:
        df = pd.DataFrame(self.target, columns=self.target_names, index=self.time_index)
        for name in self._static_features:
            data = self._features[name]
...
class TimeSeriesCorpus(Sequence[TimeSeries]):
    def __init__(self,
        ...
        if scopes is None:
            self.scopes = [(ts_id, slice(len(ts))) for ts_id, ts in enumerate(self.instances)]
        else:
            self.scopes = scopes
        self.add_series_id = add_series_id

    def _check_consistency(self, instances: List[TimeSeries]) -> List[TimeSeries]:
    ...
    def __getitem__(self, index: int) -> TimeSeries:
        ...
        if self.add_series_id:
            ts.add_features('id', np.array(ts_id), data_type='cat', feature_type='static')
        return ts

    def __getattr__(self, key: str):
        value = getattr(super(TimeSeriesCorpus, self).__getattribute__('instances')[0], key)
    ...
    def cardinalities(self) -> Dict:
        cardi = {name: len(enc.classes_) for name, enc in self.categorical_encoders.items()}
        if self.add_series_id:
            cardi['id'] = len(self)
        return cardi

    def split_by_timestamp(self,
            split_timestamp: pd.Timestamp,
            horizon: Optional[int] = None,
            context: Optional[int] = None,
            n_rolls: int = 1,
            shift: Optional[int] = None,
            raise_error: bool = True) -> Tuple[TimeSeriesCorpus, TimeSeriesCorpus]:
        ...
        for ts_id, scope in self.scopes:
            ...
            if not ((start <= test_start) and (test_stop <= stop)):
                err_msg = (f"Scope {ts_id}, requested {test_start}:{test_stop} is not "
    ...
    def split_from_end(self,
        ...
        for ts_id, scope in self.scopes:
            ...
            start = scope.start or 0
            test_start = scope.start if context is None else new_start-context
            if test_start < start:
                err_msg = (f"Scope {ts_id}, requested {test_start}:{scope.stop} is not "
    ...
    def split_from_end_by_blocks(self,
            horizon: int,
            context: Optional[int] = None,
            n_rolls: int = 1,
            n_blocks: int = 1,
        ...
        for ts_id, scope in self.scopes:
            ...
            for i in range(n_blocks):
                ...
                if test_start < start:
                    err_msg = (f"Scope {ts_id}, requested {test_start}:{stop} is not "
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        t.add_features('f3', np.random.randint(100,200,(100,)), 'cat', 'revealed')
        t.add_features('f4', np.random.randint(1000,1200,(100,)), 'cat', 'observed')
