...
class AttentionEstimator(nn.Module):
    def __init__(self,
        ...
        if self.normalize_input:
            self._normalizer = ZScoreNormalizer(rescale_loss=self.rescale_output)
    ...
    def _create_modules(self, *modules):
        self.blocks = nn.ModuleList([AttentionBlock(*ms) for ms in zip(*modules)])
    ...
    def register_loss_func(self, func: LossFunction) -> None:
        if not isinstance(func, LossFunction):
            raise ValueError('Expect a `LossFunction` object, '
        ...
        self._loss_hook_handle = self.register_forward_hook(self._loss_func.loss_hook)
    ...
    def _rollout(self,
        ...
        if not self.training:
            ...
            for step in range(total_steps):
                ...
                if feats is not None:
                    prefix_feats = pt.narrow(feats, dim=1, start=0, length=data.size(1))
                if mask is not None:
                    mask = pt.cat([mask, pt.zeros_like(pt.narrow(mask, dim=1, start=0, length=1))], dim=1)
    ...
    def forward(self,
    ...
    ) -> Tuple[Tensor, Tensor, Tensor, Optional[BoolTensor], Optional[BoolTensor]]:
        ...
        if isinstance(self.horizon, int):
            ...
            if length is None:
                horizon = data.new_ones(batch_size, dtype=pt.long) * int(full_length * self.horizon)
            else:
        ...
        if self.training:
            ...
            if mask is not None:
                ...
                fc_mask = pt.logical_or(fc_mask, mask[:, full_length-max_horizon:full_length])
            else:
            ...
            if length is not None:
                ...
                selected_index = pt.arange(full_length).to(horizon).unsqueeze(dim=0) + start_index
            ...
            if mask is not None:
                bc_mask, fc_mask = pt.split(mask, [cond_length, max_horizon], dim=1)
            else:
                bc_mask = fc_mask = None
            residue, forecast = self._rollout(data, feats, bc_mask, max_horizon)
    ...
    def from_configs(cls,
        ...
        kernel = ExpKernel(
            d_hidden, n_head, n_enc_layer, n_dec_layer,
            symmetric=symmetric, share_values=share_values, 
            dropout=dropout, temperature=temperature,
            norm=norm, add_dist=add_dist,
        )
        encoder = EncoderModule(
            d_data, d_feats, d_hidden, 
            kernel.d_head if share_values else d_hidden,
            *window_size, tie_casts=tie_casts,
        )
    ...
    def create_twin_estimator(self,
                              d_data: int,
                              d_feats: int,
                              **distinct_configs,
    ):
        kernels = [block.kernel for block in self.blocks]
        d_value = kernels[0].d_head if kernels[0].share_values else kernels[0].d_hidden
        d_hidden = kernels[0].d_hidden
        window_size = distinct_configs.get('window_size', self.blocks[0].encoder.window_size)
        tie_casts = distinct_configs.get('tie_casts', self.blocks[0].encoder.tie_casts)
        estimator_configs = dict(
            horizon = distinct_configs.get('horizon', self.horizon),
            loss_func = distinct_configs.get('loss_func', type(self._loss_func)()),
            normalize_input = distinct_configs.get('normalize_input', self.normalize_input),
            rescale_output = distinct_configs.get('rescale_output', self.rescale_output),
            layerwise_loss = distinct_configs.get('layerwise_loss', self.layerwise_loss),
        )
        
        encoder = EncoderModule(
            d_data, d_feats, d_hidden, d_value,
            *window_size,
            tie_casts=tie_casts,
        )
...
class AdversarialEstimator(AttentionEstimator):
    ...
    def _create_modules(self, *modules):
        self.blocks = nn.ModuleList([AdversarialBlock(*ms) for ms in zip(*modules)])
    ...
    def discriminative_named_parameters(self) -> Iterator[Tuple[str, Parameter]]:
        for i, block in enumerate(self.blocks):
    ...
    def from_configs(cls,
        ...
        disc = SimpleDiscriminator(
            d_model=d_hidden, n_layer=n_disc_layer,
        )
        encoder = EncoderModule(
            d_data, d_feats, d_hidden, 
            kernel.d_head if share_values else d_hidden,
    ...
    def from_base(cls,
        ...
        disc = SimpleDiscriminator(d_model=kernels[0].d_hidden, n_layer=n_disc_layer)
        ...
        if base.tie_layers:
            ...
            discs = [deepcopy(disc) for _ in range(n_layer)]
        return cls(
            encoders, kernels, decoders, discs,
    ...
    def create_twin_estimator(self,
        ...
        if self.tie_layers:
            ...
            encoders = [deepcopy(encoder) for _ in range(self.n_layer)]
            decoders = [deepcopy(decoder) for _ in range(self.n_layer)]
        return type(self)(encoders, kernels, decoders, discs, **estimator_configs)