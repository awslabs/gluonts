...
def _test_inference_identical_to_homogenous(
        ...
        n_data=20, n_timesteps=10):
    torch.manual_seed(42)
    model_inhomogenous_batch, model_inhomogenous, model_homogenous, data = \
        _make_models_and_data(device=device, dtype=dtype, n_data=n_data,
                              n_timesteps=n_timesteps)

    m_fb_ib, V_fb_ib, Cov_fb_ib = \
        model_inhomogenous_batch.smooth_forward_backward(**data)
    m_gl_ib, V_gl_ib, Cov_gl_ib = \
        model_inhomogenous_batch.smooth_global(**data)

    m_fb_i, V_fb_i, Cov_fb_i = \
        model_inhomogenous.smooth_forward_backward(**data)

    m_fb_h, V_fb_h, Cov_fb_h = \
        model_homogenous.smooth_forward_backward(**data)
    m_gl_h, V_gl_h, Cov_gl_h = \
        model_homogenous.smooth_global(**data)
...
def _test_loss_identical_to_homogenous(tolerance=2e-5, dtype=torch.float32,
                                       device="cuda:0",
                                       n_data=20, n_timesteps=10):
    model_inhomogenous_batch, model_inhomogenous, model_homogenous, data = \
        _make_models_and_data(device=device, dtype=dtype, n_data=n_data,
...
def test_loss_identical_to_homogenous_cpu_float64(tolerance=1e-12):
    _test_loss_identical_to_homogenous(tolerance=tolerance, device="cpu",
                                       dtype=torch.float64)


def test_loss_identical_to_homogenohs_gpu_float32(tolerance=1e-4):
    _test_loss_identical_to_homogenous(tolerance=tolerance, device="cuda",
                                       dtype=torch.float64)
