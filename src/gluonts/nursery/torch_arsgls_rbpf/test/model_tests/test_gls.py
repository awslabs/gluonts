...
def _make_model_and_data(device, dtype, n_timesteps=10, n_data=20, seed=42):
    ...
    model = GaussianLinearSystemHomogenous(
        n_obs=1,
        n_state=2,
        n_ctrl_state=1,
        n_ctrl_obs=3,
...
def _test_inference_identical(tolerance=2e-5, dtype=torch.float32,
                              device="cuda"):
...
def test_inference_identical_cpu_float64(tolerance=1e-10):
    _test_inference_identical(tolerance=tolerance, device="cpu",
                              dtype=torch.float64)


def test_inference_identical_gpu_float32(tolerance=1e-3):
    _test_inference_identical(tolerance=tolerance, device="cuda",
                              dtype=torch.float64)


def test_loss_identical_cpu_float64(tolerance=1e-10):
    _test_loss_identical(tolerance=tolerance, device="cpu", dtype=torch.float64)


def test_loss_identical_gpu_float32(tolerance=1e-10):
    _test_loss_identical(tolerance=tolerance, device="cuda",
                         dtype=torch.float64)
