...
from torch_extensions.ops import matvec, batch_diag_matrix, batch_diag, cholesky
...
class AuxiliarySwitchingLinearDynamicalSystem(DynamicalSystem):
    def __init__(
        ...
        if input_transformer is None:
            def none_inputs(u_static_cat=None, u_time=None):
                assert u_static_cat == None, 'controls were given, however, input transform is None'
                assert u_time == None, 'controls were given, however, input transform is None'
    ...
    def forward(self, y, u_static_cat=None, u_time=None,
        ...
        if make_forecast:
            ...
            log_weights_filter, s_filter, z_filter, m_filter, V_filter, gls_params_filter = filter_results
            y_filter_dist = self.measurement_model(torch.stack(z_filter, dim=0))
    ...
    def filter_forward(self, y, u_static_cat=None, u_time=None,
        ...
        log_weights, m, V, s, z, gls_params = ([None] * dims.timesteps for _ in
                                               range(6))
            ...
            if t == 0:
                ...
                log_weights[t], s[t], z[t], m[t], V[t], gls_params[t] = \
                    self._filter_forward_step(
                        ...
                        s=s[t - 1], z=z[t - 1], m=m[t - 1], V=V[t - 1],
                        y=y[t], u_state=u_state[t], u_obs=u_obs[t],
                        u_switch=u_switch[t],
                        seasonal_indicators=seasonal_indicators[t],
                    )
    ...
    def sample(self, s=None, x=None,
               ...
               n_timesteps=None, n_batch=None, deterministic=False):
        ...
        if initial_provided:
            ...
            state_prior = self.state_prior_model(
                None, batch_shape_to_prepend=(dims.particle, dims.batch))
            x_initial = state_prior.mean if deterministic else state_prior.rsample()
            s_initial = None
    ...
    def filter_forecast(self, n_steps_forecast, y, u_time=None,
                        u_static_cat=None,
                        seasonal_indicators=None, deterministic=False):
        dims = self.get_dims(y=y)
        n_steps_filter, n_batch = dims.timesteps, dims.batch
        n_steps_total = n_steps_filter + n_steps_forecast
        ...
        # Filter
        log_weights_filter, s_filter, z_filter, m_filter, V_filter, gls_params_filter = \
            self.filter_forward(
                y=y[:n_steps_filter],
                seasonal_indicators=seasonal_indicators[:n_steps_filter]
                if seasonal_indicators is not None else None,
        ...
        # Resample
        log_weights, s, z, m, V = \
            log_weights_filter[-1], s_filter[-1], z_filter[-1], m_filter[-1], \
            V_filter[-1]

        log_norm_weights, (s, z, m, V) = resample(
            n_particle=dims.particle,
            log_norm_weights=normalize_log_weights(
                log_weights=log_weights if not deterministic
        ...
        # Forecast
        x = MultivariateNormal(loc=m,
                               covariance_matrix=V).rsample() if not deterministic else m
        ...
        assert all([isinstance(item, (list, tuple, set)) for item in
                    [s_filter, s_forecast, m_filter, V_filter]])
        ...
        return log_norm_weights_trajectory, s_trajectory, z_trajectory, \
               y_trajectory_dist, gls_params_trajectory

    def _filter_forward_step(
            self, dims, log_weights, s, z, m, V,
        ...
        m_tp1, V_tp1 = filter_forward_measurement_step(
            y=z_tp1, m=mp, V=Vp, Q=gls_params_tp1.Q, C=gls_params_tp1.C,
            d=gls_params_tp1.d)

        measurement_dist = self.measurement_model(z_tp1)

        log_update = measurement_dist.log_prob(y) \
    ...
    def _sample_step(self, dims, s, x, seasonal_indicators, u_state=None,
                     u_obs=None,
                     u_switch=None, deterministic=False):
        switch_model_dist_tp1 = self._make_switch_model_dist(
            dims=dims, s=s, u_switch=u_switch, x=x)
        s_tp1 = switch_model_dist_tp1.mean if deterministic else switch_model_dist_tp1.rsample()
        gls_params_tp1 = self.gls_base_parameters(
            switch=s_tp1,
            seasonal_indicators=seasonal_indicators,
            u_state=u_state,
        ...
        try:
            x_tp1_dist = torch.distributions.MultivariateNormal(
                loc=(matvec(gls_params_tp1.A,
                            x) if gls_params_tp1.A is not None else x)
                    + (
    ...
    def _make_switch_proposal_dist(self, switch_model_dist,
                                   switch_encoder_dist):
    ...
    def _make_switch_model_dist(self, dims, s, u_switch, x=None, m=None,
        ...
        if s is None:  # initial step --> initial prior
            shp_prepend = (dims.particle, dims.batch) if u_switch is None else (
                dims.particle,)
    ...
    def _make_auxiliary_proposal_dist(self, auxiliary_model_dist,
                                      auxiliary_encoder_dist):
    ...
    def _make_auxiliary_model_dist(self, mp, Vp, gls_params):
        ...
        auxiliary_model_dist = MultivariateNormal(loc=mpz_tp1,
                                                  scale_tril=cholesky(Vpz_tp1))
...
class SGLSPredictor(RepresentablePredictor):
    ...
    def predict(
        ...
        for batch_gts in inference_loader:
            ...
            losses_filter_time_batch_wise, y_forecast_dist, y_filter_dist = \
            ...
            for idx_sample_in_batch, (
                    _fcst_gts, _loss_gts, _y_filter_gts) in enumerate(
                zip(forecast_gts, losses_gts, y_filter_gts)):
