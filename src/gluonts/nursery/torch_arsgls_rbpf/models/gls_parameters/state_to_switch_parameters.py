...
from models.gls_parameters.switch_link_functions import IdentityLink, \
    IndividualLink, SharedLink
from torch_extensions.ops import batch_diag_matrix, matmul


class StateToSwitchParams(nn.Module):
    def __init__(self, n_switch, n_state, n_base_F, n_base_S,
        ...
        self.F = nn.Parameter(
            torch.nn.init.orthogonal_(torch.empty(n_base_F, n_switch, n_state)),
            requires_grad=True,
        )
        ...
        if isinstance(init_scale_S_diag, (list, tuple)):
            ...
            def make_lower_logdiag(init_scale, n_base):
                ...
                Linv_logdiag = -torch.linspace(log_scale[0], log_scale[1],
                                               n_base)
            ...
            self.LSinv_logdiag = nn.Parameter(
                torch.stack([make_lower_logdiag(init_scale=init_scale_S_diag,
                                                n_base=n_base_S)
                             for _ in range(n_switch)], dim=-1),
    ...
    def forward(self, switch):
        ...
        LS_basemats = torch.inverse(torch.tril(self.LSinv_tril, -1)
                                    + batch_diag_matrix(
            torch.exp(self.LSinv_logdiag)))
