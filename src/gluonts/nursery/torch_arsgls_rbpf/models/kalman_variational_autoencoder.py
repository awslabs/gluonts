...
from inference.analytical_gausian_linear.inference_sequence_inhomogenous import \
    filter_forward, smooth_forward_backward, loss_em
...
class KalmanVariationalAutoEncoder(DynamicalSystem):
    def __init__(
            self, n_state: int, n_obs: int, n_auxiliary, n_ctrl_state: int,
            n_particle: int,
    ...
    def loss_em(self, y, u_static_cat=None, u_time=None,
                rao_blackwellized=True):
        if rao_blackwellized:
            return self._loss_em_rb(y=y, u_static_cat=u_static_cat,
                                    u_time=u_time)
        else:
            return self._loss_em_mc(y=y, u_static_cat=u_static_cat,
                                    u_time=u_time)
    ...
    def _loss_em_rb(self, y, u_static_cat=None, u_time=None):
        ...
        u = self.expand_particle_dim(self.input_transformer(
            u_static_cat=u_static_cat, u_time=u_time))
        ...
        z_initial = self.z_initial[None, None, None, :].repeat(1, dims.particle,
                                                               dims.batch, 1)
        ...
        l_em = loss_em(
            ...
            LRinv_tril=LRinv_tril[:-1], LRinv_logdiag=LRinv_logdiag[:-1],
            C=gls_params.C,
            D=gls_params.D,
            LQinv_tril=LQinv_tril, LQinv_logdiag=LQinv_logdiag,
            LV0inv_tril=LV0inv_tril, LV0inv_logdiag=LV0inv_logdiag, m0=m0,
            y=z_time_first, u_state=u.state, u_obs=u.obs,
        ).sum(
            dim=0) / dims.particle  # loss_em fn already sums over time. Only avg Particle dim.
        l_measurement = -self.measurement_model(z_particle_first).log_prob(
            y).sum(
            dim=(0, 1)) / dims.particle  # Time and Particle
        l_auxiliary_encoder = q.log_prob(z_particle_first).sum(
            dim=(0, 1)) / dims.particle  # Time and Particle
        assert all(
            l.shape == l_measurement.shape for l in
            (l_measurement, l_auxiliary_encoder, l_em))
        l_total = self.reconstruction_weight * l_measurement + l_auxiliary_encoder + l_em
        return l_total
    ...
    def _loss_em_mc(self, y, u_static_cat=None, u_time=None):
        ...
        x_smoothed_dist, z_filter_dist, gls_params, last_rnn_state, inv_measurement_dist, z = \
        ...
        transition_dist = MultivariateNormal(
            loc=matvec(gls_params.A[:-1], x[:-1]) + (
                matvec(gls_params.B[:-1],
                       u.state[:-1]) if gls_params.B is not None else 0.0),
        ...
        auxiliary_predictive_dist = MultivariateNormal(
            loc=matvec(gls_params.C, x) + (
                matvec(gls_params.D,
                       u.obs) if gls_params.D is not None else 0.0),
            covariance_matrix=gls_params.Q,
        )

        # A4) SSM related losses
        l_prior = - prior_dist.log_prob(x[0:1]).sum(
            dim=(0, 1)) / dims.particle  # time and particle dim
        l_transition = - transition_dist.log_prob(x[1:]).sum(
            dim=(0, 1)) / dims.particle  # time and particle dim
        l_auxiliary = - auxiliary_predictive_dist.log_prob(z).sum(
            dim=(0, 1)) / dims.particle  # time and particle dim
        l_entropy = x_smoothed_dist.log_prob(x).sum(  # negative entropy
            dim=(0, 1)) / dims.particle  # time and particle dim
        ...
        l_measurement = - measurement_dist.log_prob(y).sum(
            dim=(0, 1)) / dims.particle  # time and particle dim
        l_inv_measurement = inv_measurement_dist.log_prob(z_particle_first).sum(
            dim=(0, 1)) / dims.particle  # time and particle dim

        assert all(t.shape == l_prior.shape for t in
                   (l_prior, l_transition, l_auxiliary, l_measurement,
                    l_inv_measurement))
        l_total = (
                self.reconstruction_weight * l_measurement + l_inv_measurement \
                + l_auxiliary + l_prior + l_transition + l_entropy)
        return l_total
    ...
    def filter_forward(self, y, u_static_cat=None, u_time=None):
        ...
        gls_params, last_rnn_state = \
            self.compute_gls_params(
                rnn_inputs=rnn_inputs,
                u_state=u.state,
                u_obs=u.obs,
            )
        ...
        m_fw, V_fw = filter_forward(
            ...
            B=gls_params.B[:-1] if gls_params.B is not None else None,
            LRinv_tril=LRinv_tril[:-1], LRinv_logdiag=LRinv_logdiag[:-1],
            C=gls_params.C, D=gls_params.D,
            LQinv_tril=LQinv_tril, LQinv_logdiag=LQinv_logdiag,
            LV0inv_tril=LV0inv_tril, LV0inv_logdiag=LV0inv_logdiag, m0=m0,
            y=z, u_state=u.state, u_obs=u.obs,
        )
        # predictive distribution of SSM is VAE latent z.
        mpz, Vpz = filter_forward_predictive_distribution(
            m=m_fw, V=V_fw, Q=gls_params.Q, C=gls_params.C,
        ...
        return x_filter_dist, z_filter_dist, gls_params, last_rnn_state, inv_measurement_dist, z
    ...
    def smooth_forward_backward(self, y, u_static_cat=None, u_time=None):
        ...
        # predictive distribution of SSM is VAE latent z.
        mpz, Vpz = filter_forward_predictive_distribution(
            m=m_fb, V=V_fb, Q=gls_params.Q, C=gls_params.C,
            d=matvec(gls_params, u.obs) if gls_params.D is not None else None,
        )
        ...
        return x_smoothed_dist, z_filter_dist, gls_params, last_rnn_state, inv_measurement_dist, z

    def sample(self, x, z, gls_params, rnn_state,
               u_static_cat=None, u_time=None, n_timesteps=None, n_batch=None,
               deterministic=False):
        ...
        u = self.expand_particle_dim(self.input_transformer(
            u_static_cat=u_static_cat, u_time=u_time))
        dims = self.get_dims(u_state=u.state, u_obs=u.obs,
                             n_timesteps=n_timesteps, n_batch=n_batch)

        assert len(set(
            tensor is None for tensor in (x, z, rnn_state, gls_params))) == 1
        ...
        (x, z, rnn_state, gls_params) = ([None] * dims.timesteps for _ in
                                         range(4))
        for t in range(n_timesteps):
            x[t], z[t], gls_params[t], rnn_state[t] = self._sample_step(
                ...
                gls_params_tm1=gls_params[
                    t - 1] if t > 0 else gls_params_initial,
        ...
        gls_params = Box({
            name: [param[name] for param in gls_params] for name in
            gls_params[0].keys()
        })
        gls_params = Box({
            name: torch.stack(params, dim=0) if not all(
                p is None for p in params) else None
            for name, params in gls_params.items()
        })
        return x, z, gls_params, rnn_state

    def _sample_step(self, x_tm1, z_tm1, gls_params_tm1, rnn_state_tm1,
                     u_state_t, u_obs_t,
                     deterministic=False):
        ...
        x_t_dist = torch.distributions.MultivariateNormal(
            loc=(matvec(gls_params_tm1.A, x_tm1)
                 if gls_params_tm1.A is not None else x_tm1
                 ) + (
...
class KVAEPredictor(RepresentablePredictor):
    ...
    def predict(
        ...
        for batch_gts in inference_loader:
            ...
            batch_forecast = {
                name: data[n_steps_filter:] for name, data in batch.items() if
                name is not "y"
            }

            x_filter_dist, z_filter_dist, gls_params, last_rnn_state, inv_measurement_dist, z = \
                self.model.filter_forward(
                    **batch_filter)  # last time-step same as with smoothing.
            # 2) forecast --> sample
            z_filter = z_filter_dist.sample()
            x_forecast, z_forecast, gls_params_forecast, rnn_state_forecast = self.model.sample(
                x=x_filter_dist.sample()[-1],
                z=z_filter[-1],
                gls_params=Box({
                    key: val[-1] if val is not None else None for key, val in
                    gls_params.items()
                }),
            ...
            for idx_sample_in_batch, _fcst_gts, in enumerate(forecast_gts, ):
                yield SampleForecast(
                    samples=_fcst_gts,
                    start_date=batch_gts["forecast_start"][idx_sample_in_batch],
                    freq=self.freq,
