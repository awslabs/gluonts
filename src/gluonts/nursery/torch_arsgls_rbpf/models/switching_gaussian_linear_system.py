...
from utils.utils import TensorDims, create_zeros_log_weights, \
    list_of_dicts_to_dict_of_list
from inference.smc.resampling import resample, \
    make_criterion_fn_with_ess_threshold, systematic_resampling_indices
from inference.smc.normalize import normalize_log_weights
from inference.analytical_gausian_linear.inference_step import \
    filter_forward_prediction_step, filter_forward_measurement_step, \
    filter_forward_predictive_distribution
from torch_extensions.ops import matvec, batch_diag_matrix, batch_diag, \
    cholesky, vectorize_normal_dist_params
...
from torch_extensions.distributions.parametrised_distribution import \
    ParametrisedDistribution, ParametrisedMultivariateNormal, prepend_batch_dims
...
class SwitchingLinearDynamicalSystem(DynamicalSystem):
    def __init__(
            self, n_state: int, n_obs: int, n_ctrl_state: int, n_particle: int,
            n_switch: int,
        ...
        super().__init__(
            n_state=n_state, n_obs=n_obs, n_ctrl_state=n_ctrl_state,
    ...
    def forward(self, y, u_static_cat=None, u_time=None,
                seasonal_indicators=None, rsample=True,
                make_forecast=False, n_steps_forecast=None):
        ...
        if make_forecast:
            ...
            del dims
            assert all(len(u) == n_steps_total for u in
                       [u_static_cat, u_time, seasonal_indicators] if
                       u is not None)
            ...
            filter_results = \
                self.filter_forward(
                    ...
                    if seasonal_indicators is not None else None,
                    u_static_cat=u_static_cat[
                                 :n_steps_filter] if u_static_cat is not None else None,
                    u_time=u_time[
                           :n_steps_filter] if u_time is not None else None,
                    rsample=rsample,
                )
            log_weights_filter, s_filter, m_filter, V_filter, mpy_filter, Vpy_filter, gls_params_filter = filter_results

            # Resample
            log_weights, s, m, V = log_weights_filter[-1], s_filter[-1], \
                                   m_filter[-1], V_filter[-1]
            ...
            log_norm_weights, (s, m, V) = resample(
                ...
                criterion_fn=make_criterion_fn_with_ess_threshold(
                    min_ess_ratio=1.0),  # always
            )
            # Sample from last state
            x = MultivariateNormal(loc=m, scale_tril=cholesky(V)).sample()

            # Sample from generative model forward
            s_forecast, x_forecast, y_forecast, gls_params_forecast = self.sample(
                s=s, x=x,  # single (last filter) time-step.
                u_static_cat=u_static_cat[
                             n_steps_filter:] if u_static_cat is not None else None,
            ...
            return self.loss_forward(
                ...
                seasonal_indicators=seasonal_indicators, rsample=rsample,
            )

    def loss_forward(self, y, u_static_cat=None, u_time=None,
                     seasonal_indicators=None, rsample=True):
        ...
        log_conditional_likelihoods_batchwise = torch.stack([
            torch.logsumexp(lws, dim=0) for lws in log_weights
        ], dim=0)
        return -log_conditional_likelihoods_batchwise

    def filter_forecast(self, n_steps_forecast,
                        y, u_time=None, u_static_cat=None,
        ...
        assert all(len(u) == n_steps_total for u in
                   [u_time, u_static_cat, seasonal_indicators] if u is not None)

        # Filter
        log_weights_filter, s_filter, m_filter, V_filter, mpy_filter, Vpy_filter, gls_params_filter = \
            self.filter_forward(
                y=y[:n_steps_filter],
                seasonal_indicators=seasonal_indicators[:n_steps_filter]
                if seasonal_indicators is not None else None,
        ...
        # Resample
        log_weights, s, m, V = log_weights_filter[-1], s_filter[-1], m_filter[
            -1], V_filter[-1]
        log_norm_weights = normalize_log_weights(log_weights=log_weights)
        log_norm_weights, (s, m, V) = resample(
        ...
        # Forecast
        s_forecast, m_forecast, V_forecast, _, _, gls_params_forecast = self.forecast(
            s=s, m=m, V=V,  # single (last filter) time-step.
            u_static_cat=u_static_cat[
                         n_steps_filter:] if u_static_cat is not None else None,
            u_time=u_time[n_steps_filter:] if u_time is not None else None,
            seasonal_indicators=seasonal_indicators[n_steps_filter:]
        ...
        assert all([isinstance(item, (list, tuple, set)) for item in
                    [s_filter, s_forecast, m_filter, m_forecast, V_filter,
                     V_forecast]])
        ...
        for name in gls_params_filter.keys():
            if gls_params_filter[name] is not None \
                    and not all(
                param is None for param in gls_params_filter[name]):
                gls_params_trajectory[name] = torch.stack(
                    tuple(gls_params_filter[name])
                    + tuple(gls_params_forecast[name]), dim=0)
            else:
                gls_params_trajectory[name] = None
        # Predictive distribution
        predictive_dist_params = [filter_forward_predictive_distribution(
            m=m_trajectory[t],
            V=V_trajectory[t],
            Q=gls_params_trajectory.Q[t],
            C=gls_params_trajectory.C[t],
            d=gls_params_trajectory.d[
                t] if gls_params_trajectory.d is not None else None,
        ...
        return log_norm_weights_trajectory, s_trajectory, m_trajectory, V_trajectory, \
               mpy_trajectory, Vpy_trajectory, gls_params_trajectory

    def filter_forward(self, y, u_static_cat=None, u_time=None,
                       seasonal_indicators=None, rsample=True):
        u = self.input_transformer(u_static_cat=u_static_cat, u_time=u_time)
        dims = self.get_dims(y=y)
        dtype, device = self.state_prior_model.m.dtype, self.state_prior_model.m.device
        ...
        log_weights, m, V, mpy, Vpy, s, gls_params = ([None] * dims.timesteps
                                                      for _ in range(7))
        ...
        for t in range(dims.timesteps):
            if t == 0:
                ...
                log_weights[t], s[t], m[t], V[t], mpy[t], Vpy[t], gls_params[
                    t] = \
                    self._filter_forward_step(
                        dims=dims, rsample=rsample,
                        log_weights=create_zeros_log_weights(
                            dims=dims, device=device, dtype=dtype)[0],
                        s=None, m=state_prior.loc,
                        V=state_prior.covariance_matrix,
                        y=y[t], u_state=u_state[t], u_obs=u_obs[t],
                        u_switch=u_switch[t],
    ...
    def forecast(self, s, m, V,
                 u_static_cat=None, u_time=None, seasonal_indicators=None,
                 n_timesteps=None, n_batch=None):
        """
        ...
        """
        assert len(set(tensor is None for tensor in (s, m, V))) == 1, \
        ...
        if initial_provided:
            ...
            state_prior = self.state_prior_model(
                None, batch_shape_to_prepend=(dims.particle, dims.batch))
            (m_initial, V_initial) = (
                state_prior.loc, state_prior.covariance_matrix)
            s_initial = None
        (u_state, u_obs, u_switch, seasonal_indicators) = \
            ([None] * dims.timesteps if u is None else u
             for u in (u_state, u_obs, u_switch, seasonal_indicators))
        (s, m, V, mpy, Vpy, gls_params) = ([None] * dims.timesteps for _ in
                                           range(6))

        for t in range(n_timesteps):
            s[t], m[t], V[t], mpy[t], Vpy[t], gls_params[
                t] = self._forecast_step(
                dims=dims,
                s=s[t - 1] if t > 0 else s_initial,
                m=m[t - 1] if t > 0 else m_initial,
                V=V[t - 1] if t > 0 else V_initial,
                u_state=u_state[t], u_obs=u_obs[t], u_switch=u_switch[t],
                seasonal_indicators=seasonal_indicators[t],
            )
        return s, m, V, mpy, Vpy, list_of_dicts_to_dict_of_list(gls_params)

    def sample(self, s=None, x=None,
               u_static_cat=None, u_time=None, seasonal_indicators=None,
               n_timesteps=None, n_batch=None):
        assert len(set(tensor is None for tensor in (s, x))) == 1, \
    ...
    def _filter_forward_step(
            self, dims, log_weights, s, m, V,
            y, u_state=None, u_obs=None, u_switch=None,
            seasonal_indicators=None,
            rsample=True,
    ):
        log_norm_weights = normalize_log_weights(log_weights=log_weights)
        ...
        switch_proposal_dist = self._make_switch_proposal_dist(
            ...
            m=m, V=V, y=y, u_state=u_state, u_obs=u_obs, u_switch=u_switch,
        )
        s_tp1 = switch_proposal_dist.rsample() if rsample else switch_proposal_dist.sample()
        ...
        log_update = predictive_distribution.log_prob(y) \
                     + switch_model_dist.log_prob(s_tp1) \
                     - switch_proposal_dist.log_prob(s_tp1)
        log_weights_tp1 = log_norm_weights + log_update

        return log_weights_tp1, s_tp1, m_tp1, V_tp1, mpy_tp1, Vpy_tp1, gls_params_tp1

    def _forecast_step(self, dims, s, m, V,
                       seasonal_indicators=None, u_state=None, u_obs=None,
                       u_switch=None):
        ...
        m_tp1, V_tp1 = filter_forward_prediction_step(
            ...
            b=gls_params_tp1.b)
        mpy_tp1, Vpy_tp1 = filter_forward_predictive_distribution(
            m=m_tp1, V=V_tp1, Q=gls_params_tp1.Q, C=gls_params_tp1.C,
            d=gls_params_tp1.d)
        return s_tp1, m_tp1, V_tp1, mpy_tp1, Vpy_tp1, gls_params_tp1

    def _sample_step(self, dims, s, x, seasonal_indicators, u_state=None,
                     u_obs=None,
                     u_switch=None):
        switch_model_dist_tp1 = self._make_switch_model_dist(
        ...
        if not hasattr(self.gls_base_parameters, "issm"):
            x_tp1 = torch.distributions.MultivariateNormal(
                loc=(matvec(gls_params_tp1.A, x)
                     if gls_params_tp1.A is not None else x
                     ) + (
                        gls_params_tp1.b if gls_params_tp1.b is not None else 0.0),
                scale_tril=cholesky(gls_params_tp1.R),
            ).sample()
        else:
            assert (batch_diag_matrix(
                batch_diag(gls_params_tp1.R)) == gls_params_tp1.R).all()
            x_tp1 = torch.distributions.MultivariateNormal(
                loc=(matvec(gls_params_tp1.A, x)
                     if gls_params_tp1.A is not None else x
                     ) + (
    ...
    def _make_switch_proposal_dist(self, switch_model_dist, m, V,
                                   y, u_state=None, u_obs=None, u_switch=None):
        """
    ...
    def _make_switch_model_dist(
        ...
        if s is None:  # initial step --> initial prior
            shp_prepend = (dims.particle, dims.batch) \
                if u_switch is None else (dims.particle,)
            ...
            switch_model_dist = self.switch_transition_model(
                ...
                if u_switch is not None else None,
                s=s,
            )
        return switch_model_dist


class CategoricalSwitchingLinearDynamicalSystem(SwitchingLinearDynamicalSystem):
    ...
    def _make_switch_proposal_dist(self, *args, **kwargs):
        if self.temperature is None:
            ...
            return StableRelaxedOneHotCategorical(
                logits=super()._make_switch_proposal_dist(
                    *args, **kwargs).logits,
                temperature=self.temperature,
            )
...
class RecurrentSwitchingLinearDynamicalSystem(SwitchingLinearDynamicalSystem):
    def _make_switch_model_dist(self, dims, s, u_switch, x=None, m=None,
                                V=None):
...
class SGLSPredictor(RepresentablePredictor):
    ...
    def predict(
        ...
        assert num_samples == self.model.n_particle, "set n_particle from outside"
        ...
        for batch_gts in inference_loader:
            ...
            losses_filter_time_batch_wise, y_forecast, mpy_filter, Vpy_filter = \
                self.parallel_model_fn(
                    **batch, make_forecast=True,
            ...
            forecast_gts = forecast_gts.squeeze(
                axis=-1)  # this is bad, but backtest requires it.

            # return iterator over both forecast object and filter objects.
            for idx_sample_in_batch, (
                    _fcst_gts, _loss_gts, _mpy_gts, _Vpy_gts) in enumerate(
                zip(forecast_gts, losses_gts, mpy_gts, Vpy_gts)):
