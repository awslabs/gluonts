...
class BallBox:
    ...
    def run(self, iterations=20, sequences=500, angle_limits=(0, 360),
            ...
            radius=3,
            flip_gravity=None, save=None, filepath='/tmp/data/box.npz',
            delay=None):
        if save:
...
def generate_dataset(seed=1234, n_timesteps_train=20, n_timesteps_test=100):
    os.environ[
        'SDL_VIDEODRIVER'] = 'dummy'  # Remove and add delay to see the videos
    scale = 1
    np.random.seed(seed=seed)

    # Create data dir
    data_path = os.path.join(consts.data_dir, consts.Datasets.box)
