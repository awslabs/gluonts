...
def gluonts_batch_to_train_pytorch(
    batch, device, dtype, dims, time_features: TimeFeatType
):
    ...
    time_feat = torch.tensor(
        batch["past_time_feat"].asnumpy(), dtype=torch.float32, device=device
    ).transpose(0, 1)
    assert categories.shape == (n_timesteps, dims.batch)
    categories = (
        one_hot(labels=torch.tensor(categories).to(torch.int64), num_classes=5)
        .to(dtype)
        .to(device=device)
    )

    seasonal_indicators = (
        torch.tensor(batch["past_seasonal_indicators"][..., 0].asnumpy())
        .transpose(0, 1)
        .to(torch.int64)
        .to(device)
    )
    seasonal_indicator_feat = (
        one_hot(
            labels=seasonal_indicators[:, :], num_classes=7, is_squeeze=True,
        )
        .to(dtype)
        .to(device)
    )
