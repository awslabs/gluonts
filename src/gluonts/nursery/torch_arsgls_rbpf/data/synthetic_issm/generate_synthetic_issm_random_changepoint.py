...
sigmas = (
    np.array([sigma * 10 for sigma in [0.1, 0.4, 0.6, 0.8, 1.0, 1.0, 0.7]])
    / 100.0
)
T = 4 * 7
    ...
    with open(file_path, "w") as file:
        ...
        for g in range(num_groups):
            for i in range(numTsPerGroup):
                ...
                for t in range(T):
                    gamma = gammas_list[
                        np.random.randint(0, len(gammas_list))
                    ][g]
                    (
                        a_t_seasonality,
                        F_t_seasonality,
...
if __name__ == "__main__":
    for n_data in [20 * i for i in range(1, 11)]:
        ...
        folder_path = os.path.join(
            consts.data_dir, "synthetic_issm_random_changepoint", str(n_data)
        )
