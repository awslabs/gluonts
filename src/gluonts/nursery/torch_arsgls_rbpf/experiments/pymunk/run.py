...
from experiments.pymunk.configs import base_config, kvae_config, asgls_config, \
    make_asgls, make_kvae
...
def train(model, loss_fn, train_loader, log_paths, config, device, dtype):
    ...
    val_data = next(
        iter(dataloaders["train"]))  # we did not make validation set...
    val_data = {name: val.to(device).to(dtype) if val is not None else None
                for name, val in val_data.items()}
    torch.save(model.state_dict(), os.path.join(log_paths.model, f"initial.pt"))
    loss_best = np.inf
    for idx_epoch in range(config.n_epochs):
        ...
        for idx_batch, batch in enumerate(train_loader):
            optimizer.zero_grad()
            batch = {name: val.to(device).to(dtype) if val is not None else None
...
def plot_results(model, model_name, test_loader, log_paths, config,
                 deterministic,
                 n_particle=100, n_data=5, max_imgs_per_row=20,
    ...
    log_norm_weights_filter, log_norm_weights_forecast = [], [], [], [], [], []
    ...
    for idx_test_data, test_data in enumerate(test_loader):
        ...
        if (idx_test_data == n_loops - 1) and (
                n_data % config.batch_size_test) != 0:
            _test_data = {
        ...
        _z_filter, _z_forecast, _y_filter, _y_forecast, \
        _log_norm_weights_filter, _log_norm_weights_forecast = all_to_numpy(
    ...
    assert len(z_filter) == len(y_filter) == len(log_norm_weights_filter) \
           == config.dims.timesteps
    assert len(z_forecast) == len(y_forecast) == len(log_norm_weights_forecast) \
           == config.prediction_length
    assert len(z_trajectory) == len(y_trajectory) == len(
        log_norm_weights_trajectory) \
           == config.dims.timesteps + config.prediction_length

    if deterministic:
        assert y_trajectory.shape[1:3] == y_filter.shape[
                                          1:3] == y_forecast.shape[1:3] \
               == z_trajectory.shape[1:3] == (1, n_data,)
    else:
        assert y_trajectory.shape[1:3] == y_filter.shape[
                                          1:3] == y_forecast.shape[1:3] \
               == z_trajectory.shape[1:3] == (n_particle, n_data,)

    # ********** Plots **********
    for idx_data in range(n_data):
        os.makedirs(os.path.join(log_paths.plot, f"B{idx_data}"), exist_ok=True)
        ...
        for idx_dim in range(z_trajectory.shape[-1]):
            if deterministic:
                ...
                plt.scatter(
                    ...
                    s=(log_norm_weights_trajectory[:, :, idx_data]
                       + np.log(n_particle) + 5).clip(0, np.inf),
                    alpha=0.5,
                )
        plt.xlabel("t")
        plt.ylabel("z")
    ...
    def plot_imgs_over_time(img, save_path):
        ...
        plt.tight_layout()
        fig.set_size_inches(1. * img.shape[1] / img.shape[0], 1, forward=False)
        plt.savefig(save_path, bbox_inches="tight", pad_inches=0.025)
        plt.close(fig)

    for idx_data in range(n_data):
        img_filt_gt = test_data_filter['y'][:n_steps_visual, idx_data,
                      :].reshape([-1, 32, 32])
        img_fcst_gt = test_data_forecast['y'][:n_steps_visual, idx_data,
                      :].reshape([-1, 32, 32])
        for idx_particle in range(n_particles_visual):
            img_filt = y_filter[:n_steps_visual, idx_particle, idx_data,
                       :].reshape([-1, 32, 32])
            img_fcst = y_forecast[:n_steps_visual, idx_particle, idx_data,
                       :].reshape([-1, 32, 32])
...
if __name__ == "__main__":
    ...
    loss_fn_kvae = lambda model, data: model.loss_em(
        **data, rao_blackwellized=kvae_config.rao_blackwellized).sum()
    ...
    print(log_paths)
    model = train(model=model, loss_fn=loss_fn,
                  train_loader=dataloaders["train"],
                  log_paths=log_paths, dtype=dtype, device=device,
                  config=config)

    print("done training")
    with torch.no_grad():
        plot_results(model=model, model_name=config.experiment_name,
                     test_loader=dataloaders["test"],
                     log_paths=log_paths,
                     config=config, deterministic=False, device=device)
        plot_results(model=model, model_name=config.experiment_name,
                     test_loader=dataloaders["test"],
                     log_paths=log_paths,
                     config=config, deterministic=True, device=device)
