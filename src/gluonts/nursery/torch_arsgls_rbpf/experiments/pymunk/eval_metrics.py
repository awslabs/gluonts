...
def compute_metrics(model, model_name, test_loader, config, n_particle=100):
    ...
    for deterministic in [False]:
        for idx_test_data, test_data in enumerate(tqdm.tqdm(test_loader)):
            ...
            test_data_filter = {
                "y": test_data["y"][: config.dims.timesteps]
                .to(dtype)
                .to(device)
            }
            test_data_forecast = {
            ...
            y_filter_groundtruth = (
                test_data_filter["y"][:, None, ...]
                .repeat([1, (1 if deterministic else n_particle), 1, 1])
                .cpu()
                .numpy()
            )
            y_forecast_groundtruth = (
                test_data_forecast["y"][:, None, ...]
                .repeat([1, (1 if deterministic else n_particle), 1, 1])
                .cpu()
                .numpy()
            )
            ...
            acc_fcst = (
                np.equal(y_forecast, y_forecast_groundtruth)
                .astype(np.float64)
                .mean(axis=-1)
            )
            acc_filt = (
                np.equal(y_filter, y_filter_groundtruth)
                .astype(np.float64)
                .mean(axis=-1)
            )
            if deterministic:
