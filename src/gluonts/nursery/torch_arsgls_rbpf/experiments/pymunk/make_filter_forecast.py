...
def make_kvae_filter_forecast(
    model, data_filter, prediction_length, n_particle, deterministic
):
    ...
    y_forecast = (
        y_forecast_dist.mean if deterministic else y_forecast_dist.sample()
    )
...
def make_arsgls_filter_forecast(
    model, data_filter, prediction_length, n_particle, deterministic
):
    n_particle_prev = model.n_particle
    ...
    y_trajectory = (
        y_trajectory_dist.mean if deterministic else y_trajectory_dist.sample()
    )
    if deterministic:
