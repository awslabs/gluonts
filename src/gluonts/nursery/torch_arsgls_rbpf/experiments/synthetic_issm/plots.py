...
def _load_past_params(epoch, model, log_paths):
    ...
    model_params = {
        name: [None] * epoch for name, val in model.named_parameters()
    }
    ...
    model_params = {
        name: np.stack(val, axis=0) for name, val in model_params.items()
    }
    return model_params
...
def plot_switch_trajectories(
    s, n_data_plot=5, n_particles_plot=1, idx_forecast_start=None
):
    ...
    n_particles_plot = (
        n_particle if n_particles_plot > n_particle else n_particles_plot
    )
    t = np.arange(n_timesteps)

    colors = plt.cm.get_cmap("viridis", n_switch)
    fig, axs = plt.subplots(
        n_data_plot, figsize=[4, n_data_plot * 2], squeeze=True
    )
    for idx_data in range(n_data_plot):
        ...
        for idx_switch in range(n_switch):
            ax.plot(
                t,
                s[:, :n_particles_plot, idx_data, idx_switch],
                color=colors(idx_switch),
                linewidth=1,
            )
        ax.set_xlabel("t")
        ax.set_ylabel("s")
    ...
    fig.suptitle(
        "switch variable samples --- color indicates switch dimension", y=1.0
    )
    return fig, axs
...
def plot_filtered_forecasted_covariances(
    ...
    t = np.array(
        tuple(
            range(
                n_timesteps - n_steps_forecast - n_steps_filter_plot,
                n_timesteps,
            )
        )
    )
    ...
    group_indices = [
        [
            idx_data
            for idx_data in range(n_batch)
            if data["u_static_cat"][idx_timestep, idx_data, idx_group] == 1
        ]
    ...
    for idx_group in range(n_groups):
        ...
        R = np.concatenate(
            [
                gls_params.R[
                    t : t + 1, :, data_indices, t % dim_state, t % dim_state
                ]
                for t in t
            ],
            axis=0,
        ...
        Q = np.concatenate(
            [gls_params.Q[t : t + 1, :, data_indices, 0, 0] for t in t], axis=0
        )
        LR = R ** 0.5  # lat noise scale
        LQ = Q ** 0.5  # obs noise scale
        weights = norm_weights[t, ...][
            ..., data_indices
        ]  # cannot index simultanously
        ...
        LR_gt = [
            lat_noise_scale_groundtruth[idx_group, t % dim_state] for t in t
        ]
        LQ_gt = [
            obs_noise_scale_groundtruth[idx_group, t % dim_state] for t in t
        ]

        colors = plt.cm.get_cmap("viridis", len(data_indices))
        if n_steps_filter_plot != 0:
            axR_aggregate.axvline(
                t[-1] - n_steps_forecast, color="black", linestyle="--"
            )
            axR_samples.axvline(
                t[-1] - n_steps_forecast, color="black", linestyle="--"
            )
            axQ_aggregate.axvline(
                t[-1] - n_steps_forecast, color="black", linestyle="--"
            )
            axQ_samples.axvline(
                t[-1] - n_steps_forecast, color="black", linestyle="--"
        ...
        miniR, maxiR = (
            lat_noise_scale_groundtruth.min(),
            lat_noise_scale_groundtruth.max(),
        )
        ...
        miniQ, maxiQ = (
            obs_noise_scale_groundtruth.min(),
            obs_noise_scale_groundtruth.max(),
        )
        axQ_samples.set_ylim([miniQ - 0.05, maxiQ + 0.05])
        axQ_aggregate.set_ylim([miniQ - 0.05, maxiQ + 0.05])

    figR.suptitle(
        "latent noise scale: left samples, right mean and 3*std", y=1.0
    )
    figQ.suptitle(
        "observation noise scale: left samples, right mean and 3*std", y=1.0
    )
...
def make_params_over_training_plots(epoch, model, log_paths, show=False):
    model_params = _load_past_params(
        epoch=epoch, model=model, log_paths=log_paths
    )
    state_noise_scale = np.exp(
        -model_params["gls_base_parameters.LRinv_logdiag"]
    )
    obs_noise_scale = np.exp(
        -model_params["gls_base_parameters.LQinv_logdiag"]
    )
    ...
    _savefig(
        fig=fig,
        fname=os.path.join(log_paths.plot, "state_prior_during_training.pdf"),
    )
    ...
    _savefig(
        fig=fig,
        fname=os.path.join(log_paths.plot, "state_noise_during_training.pdf"),
    )
    ...
    _savefig(
        fig=fig,
        fname=os.path.join(
            log_paths.plot, "observation_noise_during_training.pdf"
        ),
    )
...
def make_state_prior_plots(model, log_paths, show=False):
    ...
    fig, axs = plot_state_prior_final(
        model_params=model_params,
        m0_groundtruth=m0_groundtruth,
        sigma0_groundtruth=sigma0_groundtruth,
    )
    _savefig(
        fig=fig, fname=os.path.join(log_paths.plot, "state_prior_final.pdf")
    )
    if show:
        plt.show()
    plt.close()
...
def make_forecast_plots(
    epoch, model, log_paths, dims, n_steps_forecast, data, show=False
):
    log_norm_weights, s, m, V, mpy, Vpy, gls_params = model.filter_forecast(
        n_particle=dims.particle, n_steps_forecast=n_steps_forecast, **data
    )
    ...
    _savefig(
        fig=fig,
        fname=os.path.join(log_paths.plot, f"switch_trajectory_ep{epoch}.pdf"),
    )

    assert generate_synthetic_issm.sigmas.ndim == 1, (
        generate_synthetic_issm.gammas.ndim == 2
    )
    ...
    _savefig(
        fig=figR,
        fname=os.path.join(
            log_paths.plot, f"filtered_forecasted_R_ep{epoch}.pdf"
        ),
    )
    _savefig(
        fig=figQ,
        fname=os.path.join(
            log_paths.plot, f"filtered_forecasted_Q_ep{epoch}.pdf"
        ),
    )
...
def plot_predictive_distribution(
    y, mpy, Vpy, norm_weights, idx_batch, idx_particle=None
):
    ...
    ax_p.plot(t, _mpy)
    ax_p.fill_between(
        t, _mpy - 3 * _Vpy ** 0.5, _mpy + 3 * _Vpy ** 0.5, alpha=0.25
    )
...
def plot_filtered_states(m, V, norm_weights, idx_batch, idx_particle=None):
    ...
    for idx_state in range(n_state):
        if idx_particle is None:
            _m = (
                norm_weights[:, :, idx_batch] * m[:, :, idx_batch, idx_state]
            ).sum(axis=1)
            _V = (
                np.sum(
                    (
                        norm_weights[:, :, idx_batch]
                        * V[:, :, idx_batch, idx_state, idx_state]
                    ),
                    axis=1,
                )
                + np.sum(
        ...
        ax_f.fill_between(
            ...
            _m + 3 * _V ** 0.5,
            alpha=0.25,
            color=colors(idx_state),
        )
        if idx_particle is not None:
            ax_w.plot(
                t, norm_weights[:, idx_particle, idx_batch], color="black"
            )
    ax_f.legend([f"dim {idx_state}" for idx_state in range(n_state)])
    if idx_particle is None:
        ax_f.set_title(
            f"filtered and forecasted states - "
            f"importance-weighted, data idx {idx_batch}"
        )
    else:
        ax_f.set_title(
...
if __name__ == "__main__":
    ...
    data = gluonts_batch_to_train_pytorch(
        batch=data, device="cpu", dtype=torch.float32, dims=config.dims
    )
