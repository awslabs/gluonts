...
from experiments.synthetic_issm.plots import (
    _savefig,
    plot_state_prior_final,
    plot_switch_trajectories,
    plot_filtered_states,
...
def plot_filtered_forecasted_covariances(
    ...
    for idx_group in range(n_groups):
        ...
        LR_gt = [
            lat_noise_scale_groundtruths[_t, idx_group, _t % dim_state]
            for _t in t
        ]
        LQ_gt = [
            obs_noise_scale_groundtruth[idx_group, _t % dim_state] for _t in t
        ]
        ...
        LR_log_probs = LR_normal.log_prob(
            LR_heatmap_grid[:, None, None, None]
        )[..., None].transpose(0, -1)[0]
        LR_weighted_probs = (torch.exp(LR_log_probs) * weights[..., None]).sum(
            dim=[1, 2]
        )
        axR_samples.imshow(
            LR_weighted_probs.T.numpy()[::-1],
            cmap="gray",
            interpolation="none",
            aspect="auto",
            extent=[t[0], t[-1], LR_heatmap_grid[0], LR_heatmap_grid[-1]],
        )
        ...
        LQ_log_probs = LQ_normal.log_prob(
            LQ_heatmap_grid[:, None, None, None]
        )[..., None].transpose(0, -1)[0]
        LQ_weighted_probs = (torch.exp(LQ_log_probs) * weights[..., None]).sum(
            dim=[1, 2]
        )
        axQ_samples.imshow(
            LQ_weighted_probs.T.numpy()[::-1],
            cmap="gray",
            interpolation="none",
            aspect="auto",
            extent=[t[0], t[-1], LQ_heatmap_grid[0], LQ_heatmap_grid[-1]],
        )
        ...
        miniR = np.min(
            [
                lat_noise_scale_groundtruths[idx_gt][idx_group]
...
def make_forecast_plots(
    ...
    _savefig(
        ...
        fname=os.path.join(
            log_paths.plot,
            f"filtered_state_importance_weighted_b{idx_batch}.pdf",
        ),
    )
    fig, axs = plot_predictive_distribution(
        y=y,
    ...
    assert all(
        generate_synthetic_issm_changepoint.gammas_list[idx].ndim == 2
        for idx in range(len(generate_synthetic_issm_changepoint.gammas_list))
    )

    n_groups = generate_synthetic_issm_changepoint.gammas_list[0].shape[0]

    obs_noise_scale_groundtruth = np.repeat(
        generate_synthetic_issm_changepoint.sigmas[None, :],
        repeats=n_groups,
        axis=0,
    )
    ...
    pattern_durations = np.array(tuple(change_timesteps) + (T,)) - np.array(
        (0,) + tuple(change_timesteps)
    )
