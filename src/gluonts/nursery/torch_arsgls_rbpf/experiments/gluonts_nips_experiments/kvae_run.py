...
from experiments.gluonts_nips_experiments.kvae_config import make_model, \
...
def make_plots(model, data, config, idxs_time_series, idx_epoch):
    for idx in idxs_time_series:
        ...
        mpy = torch.cat([
            y_predictive_dist.loc, y_forecast_dist.loc,
        ], dim=0)
        Vpy = torch.cat([
        ...
        plot_predictive_distribution(
            ...
            mpy=mpy.detach(),
            Vpy=Vpy.detach(),
            norm_weights=torch.ones(mpy.shape[:3],
                                    device=mpy.device,
                                    dtype=mpy.dtype) / config.num_samples_eval,
    ...
    for idx_epoch in epoch_iter:
        ...
        for idx_batch, batch in enumerate(train_loader):
            ...
            loss = model.loss_em(
                **batch, rao_blackwellized=config.rao_blackwellized
                ...
                batch['y'].shape[:2])  # regularisation is tuned for mean
            loss.backward()
        ...
        if idx_epoch == 0 or idx_epoch > 25:
            ...
            if config.dataset_name == "electricity_nips":
                idxs_time_series = [0, 8,
                                    9] if config.batch_size_val >= 3 else [0]
            else:
                idxs_time_series = [0, 1,
                                    2] if config.batch_size_val >= 3 else [0]
            make_plots(model=model, data=val_data, config=config,
                       idxs_time_series=idxs_time_series, idx_epoch=idx_epoch)
