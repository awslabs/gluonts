...
def test(model, dataset, log_paths, input_transforms, cardinalities, config,
    ...
    forecaster_test_rolling = KVAEPredictor(
        model=model,
    ...
    dataset_test_rolling = ListDataset(dataset.test, freq=dataset.metadata.freq)
    ...
    with torch.no_grad():
        ...
        np.savez(
            os.path.join(log_paths.metrics, f"agg_metrics_rolling_{which}.npz"),
            agg_metrics_rolling,
        )
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_full_loader, test_rolling_loader, input_transforms = \
        create_loaders(
            dataset=dataset,
            batch_sizes={"train": dims.batch, "val": config.batch_size_val,
