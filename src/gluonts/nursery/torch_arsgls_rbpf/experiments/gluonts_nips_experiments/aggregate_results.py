...
result_names = [
    "agg_metrics_full_final",
    "agg_metrics_rolling_final",
    "agg_metrics_full_best",
    "agg_metrics_rolling_best",
]
...
for result_name in result_names:
    for idx_f, f in enumerate(folder_paths):
        file = np.load(
            os.path.join(f, "metrics", f"{result_name}.npz"), allow_pickle=True
        )
...
for result_name in result_names:
    for metric_name in aggregated_results_mean[result_name]:
        aggregated_results_mean[result_name][metric_name] = np.mean(
...
for result_name in result_names:
    for metric_name in aggregated_results_std[result_name]:
        aggregated_results_std[result_name][metric_name] = np.std(
...
for metric_name in aggregated_results_std[result_name]:
    for result_name in result_names:
        print(
            f"{metric_name}: {result_name.split('agg_metrics_')[1]}: "
