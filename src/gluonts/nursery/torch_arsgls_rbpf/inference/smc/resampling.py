...
def repeat_like(self: torch.Tensor, target: torch.Tensor,
                self_dims_in_target: tuple):
    assert isinstance(self_dims_in_target, (tuple, list, set))
    self_dims_in_target = tuple(sorted(dim if dim >= 0 else target.ndim + dim
...
def make_argmax_log_weights(log_weights, dim: int = -2):
    ...
    is_max = torch.nn.functional.one_hot(
        torch.argmax(log_weights, dim=dim),
        num_classes=log_weights.shape[dim],
    ).transpose(dim, -1).to(torch.bool)
    argmaxed_log_weights = torch.where(
        is_max, torch.zeros_like(log_weights),
        - np.inf * torch.ones_like(log_weights)
    )
    return argmaxed_log_weights


def categorical_resampling_indices(log_norm_weights: torch.Tensor,
                                   n_particle: int, dim: int = -2):
...
def systematic_resampling_indices(log_norm_weights: torch.Tensor,
                                  n_particle: int, dim: int = -2):
    ...
    sampling_noise = torch.rand(noise_shp, dtype=dtype,
                                device=device) / n_particle
    ...
    indices = torch.le(
        ...
        # 1PB --> broadcast on "1"
    )[:-1].to(log_norm_weights.dtype).sum(
        dim=0)  # elem [-1] may not be "True". see above.
    return indices.to(torch.int64)
...
def make_criterion_fn_with_ess_threshold(min_ess_ratio: float = 0.5):
    ...
    def criterion_fn(log_norm_weights, dim):
        ...
        log_min_particles = torch.log(
            torch.tensor(min_ess_ratio * log_norm_weights.shape[dim],
...
def resample(n_particle: int, log_norm_weights: torch.Tensor,
             tensors_to_resample: Tuple[torch.Tensor, ...],
             resampling_indices_fn: callable,
             criterion_fn: callable,
             dim: int = -2):
    dim = dim if dim >= 0 else log_norm_weights.ndim + dim
    shp_resampled_weights = tuple(n if d != dim else n_particle
                                  for d, n in enumerate(log_norm_weights.shape))
    uniform_weights = torch.zeros(shp_resampled_weights,
                                  dtype=log_norm_weights.dtype,
                                  device=log_norm_weights.device) - math.log(
        n_particle)

    if n_particle != log_norm_weights.shape[dim]:  # re-sample all
        shp_needs = tuple(
            n for d, n in enumerate(log_norm_weights.shape) if d != dim)
        needs_resampling = torch.ones(shp_needs, dtype=torch.bool,
                                      device=log_norm_weights.device)
    ...
    idxs_keep = repeat_like(torch.arange(
        n_particle, dtype=idxs_resample.dtype, device=idxs_resample.device),
        idxs_resample, self_dims_in_target=(dim,),
    )
    ...
    resampled_tensors = tuple(particles.gather(
        dim=dim,
        index=repeat_like(idxs_resample_if_criterion,
                          particles,
                          self_dims_in_target=tuple(range(
                              idxs_resample_if_criterion.ndim))
                          ),
