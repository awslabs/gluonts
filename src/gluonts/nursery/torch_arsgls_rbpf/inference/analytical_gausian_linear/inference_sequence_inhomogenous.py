...
from torch_extensions.ops import batch_cholesky_inverse, batch_diag, kron, \
    symmetrize, matvec, make_block_diagonal, \
    cov_from_invcholesky_param, inv_from_invcholesky_param, cholesky
...
from utils.utils import create_zeros_state_vec, create_zeros_state_mat, \
    add_sample_dims_to_initial_state, LOG_2PI


def filter_forward(dims, A, B, C, D,
                   LQinv_tril, LQinv_logdiag,
                   LRinv_tril, LRinv_logdiag,
                   LV0inv_tril, LV0inv_logdiag,
                   m0, y, u_state=None, u_obs=None):
    ...
    for t in range(0, dims.timesteps):
        if t == 0:
            ...
            R_tm1 = cov_from_invcholesky_param(LRinv_tril[t - 1],
                                               LRinv_logdiag[t - 1])
            b_tm1 = matvec(B[t - 1], u_state[t - 1]) \
                if u_state is not None else 0
...
def smooth_forward_backward(dims, A, B, C, D,
                            LQinv_tril, LQinv_logdiag,
                            LRinv_tril, LRinv_logdiag,
                            LV0inv_tril, LV0inv_logdiag,
                            m0, y, u_state=None, u_obs=None):
    ...
    for t in reversed(range(0, dims.timesteps - 1)):
        ...
        m_sm[t], V_sm[t], Cov_sm[t] = smooth_backward_step(
            m_sm=m_sm[t + 1], V_sm=V_sm[t + 1], m_fw=m_fw[t], V_fw=V_fw[t],
            A=A[t], R=Rt, b=bt,
        )
    return torch.stack(m_sm, dim=0), torch.stack(V_sm, dim=0), torch.stack(
        Cov_sm, dim=0)


def smooth_global(dims, A, B, C, D,
                  LQinv_tril, LQinv_logdiag,
                  LRinv_tril, LRinv_logdiag,
                  LV0inv_tril, LV0inv_logdiag,
                  m0, y, u_state=None, u_obs=None):
    ...
    Q_field = torch.zeros((dims.batch, dims.timesteps * dims.state,
    ...
    for t in range(dims.timesteps - 1):
        id = t * dims.state
        h_field[:, id: id + dims.state] \
            += -matvec(RinvA[t].transpose(-1, -2), b[t])
        h_field[:, id + dims.state: id + 2 * dims.state] \
            += matvec(Rinv[t], b[t])
        Q_field[:, id: id + dims.state, id: id + dims.state] += AtRinvA[t]
        Q_field[:, id: id + dims.state, id + dims.state: id + 2 * dims.state] \
            += -RinvA[t].transpose(-1, -2)
        Q_field[:, id + dims.state: id + 2 * dims.state, id: id + dims.state] \
            += -RinvA[t]
    ...
    for t in range(0, dims.timesteps):
        ...
        if t < (dims.timesteps - 1):
            ...
            Cov.append(torch.zeros((dims.batch, dims.state, dims.state),
                                   device=device, dtype=dtype))
...
def sample(dims, A, B, C, D, LQinv_tril, LQinv_logdiag, LRinv_tril,
           LRinv_logdiag, LV0inv_tril, LV0inv_logdiag, m0, u_state=None,
           u_obs=None):
    # generate noise
    ...
    for t in range(1, dims.timesteps):
        LR_tm1 = torch.inverse(torch.tril(LRinv_tril[t - 1], -1)
                               + torch.diag(torch.exp(LRinv_logdiag[t - 1])))
        LQ_t = torch.inverse(torch.tril(LQinv_tril[t], -1)
...
def loss_forward(dims, A, B, C, D,
                 LQinv_tril, LQinv_logdiag,
                 LRinv_tril, LRinv_logdiag,
                 LV0inv_tril, LV0inv_logdiag, m0,
                 y, u_state=None, u_obs=None):
    """
    ...
    dim_particle = (dims.particle,) \
        if dims.particle is not None and dims.particle != 0 else tuple()
    loss = torch.zeros(dim_particle + (dims.batch,), device=device, dtype=dtype)
    for t in range(0, dims.timesteps):
        (mp, Vp) = filter_forward_prediction_step(
            m=m_fw[t - 1],
            V=V_fw[t - 1],
            R=cov_from_invcholesky_param(LRinv_tril[t - 1],
                                         LRinv_logdiag[t - 1]),
            A=A[t - 1],
...
def loss_em(dims, A, B, C, D,
    ...
    if A.ndim == 3:  # No Batch and Particle dimension --> Must add at least Batch dimension.
    ...
    loss_init = 0.5 * (torch.sum(V0inv * quad_init, dim=(-1, -2))  # FF
    ...
    quad_trans = matmul(dtrans, dtrans.transpose(-1, -2)) \
                 + V[1:] \
                 - matmul(A, Cov[:-1]) \
                 - matmul(Cov[:-1].transpose(-1, -2), A.transpose(-1, -2)) \
                 + matmul(matmul(A, V[:-1]), A.transpose(-1, -2))

    loss_trans = 0.5 * (torch.sum(Rinv * quad_trans, dim=(0, -1, -2))  # T...FF
    ...
    quad_obs = matmul(dobs, dobs.transpose(-1, -2)) \
               + matmul(C, matmul(V, C.transpose(-1, -2)))
    loss_obs = 0.5 * (torch.sum(Qinv * quad_obs, dim=(0, -1, -2))  # T...FF
    ...
    assert loss_trans.shape == loss_obs.shape == loss_init.shape == loss_entropy.shape
...
def compute_entropy(dims, V, Cov):
    ...
    for t in range(0, dims.timesteps):
        if t == 0:  # marginal entropy (t==0)
            LVt = cholesky(V[t])
            entropy += 0.5 * (2.0 * torch.sum(torch.log(batch_diag(LVt)),
                                              dim=(-1,))  # F
            ...
            entropy += 0.5 * (2.0 * torch.sum(torch.log(batch_diag(LCov_cond)),
                                              dim=(-1,))  # F
                              + dims.state * (1.0 + LOG_2PI))
    return entropy
