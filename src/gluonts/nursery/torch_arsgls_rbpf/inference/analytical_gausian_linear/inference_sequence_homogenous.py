...
def filter_forward(dims, A, B, C, D, LQinv_tril, LQinv_logdiag,
    ...
    V_fw = torch.zeros((dims.timesteps, dims.batch, dims.state, dims.state),
                       device=device, dtype=dtype)
...
def smooth_forward_backward(dims, A, B, C, D, LQinv_tril, LQinv_logdiag,
                            LRinv_tril, LRinv_logdiag,
                            LV0inv_tril, LV0inv_logdiag, m0,
                            y, u_state=None, u_obs=None):
    device, dtype = A.device, A.dtype
    ...
    m_sm = torch.zeros((dims.timesteps, dims.batch, dims.state),
                       device=device, dtype=dtype)
    V_sm = torch.zeros((dims.timesteps, dims.batch, dims.state, dims.state),
                       device=device, dtype=dtype)
    Cov_sm = torch.zeros((dims.timesteps, dims.batch, dims.state, dims.state),
                         device=device, dtype=dtype)
...
def smooth_global(dims, A, B, C, D, LQinv_tril, LQinv_logdiag,
                  LRinv_tril, LRinv_logdiag,
                  LV0inv_tril, LV0inv_logdiag, m0,
                  y, u_state=None, u_obs=None):
    ...
    Q_obs = kron(torch.eye(dims.timesteps, dtype=dtype, device=device),
    ...
    for t in range(dims.timesteps - 1):
        idx = t * dims.state
        h_field[:, idx: idx + dims.state] += -matvec(RinvA.transpose(-1, -2),
                                                     b[t])
        h_field[:, idx + dims.state: idx + 2 * dims.state] \
            += matvec(Rinv, b[t])
        Q_field[:, idx: idx + dims.state,
        idx: idx + dims.state] += AtRinvA
        Q_field[:, idx: idx + dims.state,
        idx + dims.state: idx + 2 * dims.state] += -RinvA.transpose(-1, -2)
...
def sample(dims, A, B, C, D, LQinv_tril, LQinv_logdiag,
           LRinv_tril, LRinv_logdiag,
           LV0inv_tril, LV0inv_logdiag, m0,
           u_state=None, u_obs=None):
    device, dtype = A.device, A.dtype
    ...
    LV0 = torch.inverse(torch.tril(LV0inv_tril, -1)
...
def loss_forward(dims, A, B, C, D, LQinv_tril, LQinv_logdiag,
                 LRinv_tril, LRinv_logdiag,
                 LV0inv_tril, LV0inv_logdiag, m0,
                 y, u_state=None, u_obs=None):
    device, dtype = A.device, A.dtype
...
def loss_em(dims, A, B, C, D, LQinv_tril, LQinv_logdiag,
            LRinv_tril, LRinv_logdiag,
            LV0inv_tril, LV0inv_logdiag, m0,
            y, u_state=None, u_obs=None):
    ...
    loss_init = 0.5 * (torch.sum(V0inv * quad_init, dim=(0, -1, -2))
    ...
    quad_trans = matmul(delta_trans.transpose(0, 1).transpose(-1, -2),
    ...
    loss_trans = 0.5 * (
            ...
            - 2.0 * (dims.timesteps - 1) * dims.batch * torch.sum(LRinv_logdiag,
                                                                  dim=-1)
    ...
    quad_obs = matmul(delta_obs.transpose(0, 1).transpose(-1, -2),
                      delta_obs.transpose(0, 1)) \
               + matmul(C, matmul(V_sum, C.transpose(-1, -2)))
    loss_obs = 0.5 * (
            torch.sum(Qinv * quad_obs, dim=(0, -1, -2))
            - 2.0 * dims.timesteps * dims.batch * torch.sum(LQinv_logdiag,
                                                            dim=-1)
