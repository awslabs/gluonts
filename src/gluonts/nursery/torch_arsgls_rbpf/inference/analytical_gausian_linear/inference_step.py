...
from torch_extensions.ops import batch_cholesky_inverse, symmetrize, matvec, \
    cholesky
...
def filter_forward_measurement_step(y, m, V, Q, C, d=None,
    ...
    S = matmul(LVpyinv, matmul(C,
                               V))  # CV is cov_T. cov VC.T could be output of predictive dist.
    dobs = y - mpy
...
def smooth_backward_step(m_sm, V_sm, m_fw, V_fw, A, b, R):
    ...
    P = (matmul(A, matmul(V_fw,
                          A.transpose(-1, -2))) if A is not None else V_fw) + R
    ...
    m_sm_t = m_fw + matvec(J, m_sm - (
        matvec(A, m_fw) if A is not None else m_fw) - b)
    V_sm_t = symmetrize(V_fw + matmul(J, matmul(V_sm - P, J.transpose(-1, -2))))
