...
from torch.distributions import MultivariateNormal, Bernoulli, \
    Categorical, OneHotCategorical, Normal, Distribution
...
class ProbabilisticSensorFusion(nn.Module):
    ...
    def forward(self, *distributions):
        ...
        fused_dist_params = self.natural_to_dist_params(fused_natural_params,
                                                        dist_cls=dist_cls)
    ...
    def get_natural_params(dists: Tuple[Distribution]):
        ...
        if dist_cls is Normal:
            return tuple({
                             ...
                             "neg_precision": -0.5 * dist.scale.pow(
                                 2).reciprocal(),
                         } for dist in dists)
    ...
    def natural_to_dist_params(natural_params: dict, dist_cls):
        ...
        """
        if dist_cls is Normal:
            eta, neg_precision = natural_params["eta"], natural_params[
                "neg_precision"]
            return {
    ...
    def sum_natural_params(natural_params: Tuple[Dict]):
        ...
        return {np_name: sum(
            natural_params[idx_param][np_name] for idx_param in range(n_dists))
...
def test_fusion_manually(distributions, threshold=1e-4):
    ...
    if type(distributions[0]) in [Categorical]:
        test_points = torch.randint(
            low=0, high=distributions[0].probs.shape[-1],
            size=(100,) + tuple(fused_dist.batch_shape) + tuple(
                fused_dist.event_shape),
        )
    elif type(distributions[0]) in [OneHotCategorical]:
        test_points = one_hot(torch.randint(
            low=0, high=distributions[0].probs.shape[-1] - 1,
            size=(100,) + tuple(fused_dist.batch_shape) + tuple(
                fused_dist.event_shape),
        ...
        test_points = torch.randn(
            (100,) + tuple(fused_dist.batch_shape) + tuple(
                fused_dist.event_shape))
    ...
    diff_log_prob_manual = manually_fused_unnormalized[
                           1:] - manually_fused_unnormalized[:-1]
...
if __name__ == "__main__":
    # TODO: put this into a test outside of this file.
    ...
    for distributions in [
        ...
        [
            MultivariateNormal(loc=torch.rand(10),
