...
class GaussianRecurrentTransition(nn.Module):
    def __init__(self, conditional_dist_tranform: nn.Module,
                 n_state: int, n_switch: int, is_recurrent: bool = True):
        super().__init__()
        self.conditional_dist_transform = conditional_dist_tranform
        if is_recurrent:
            self.F = nn.Parameter(
                # torch.nn.init.xavier_normal(torch.empty(n_switch, n_state)) / 4.0,
                torch.nn.init.orthogonal_(torch.empty(n_switch, n_state)) / 4.0,
                requires_grad=True,
            )
        else:
