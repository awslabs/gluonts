...
class ParametrisedConditionalDistribution(nn.Module):
    def __init__(self, stem: nn.Module, dist_params: nn.ModuleDict, dist_cls,
                 n_feature_dims=1, allow_cat_inputs=False):
        super().__init__()
    ...
    def forward(self, x: (torch.Tensor, tuple, list, set),
        ...
        dist_params = {
            name: prepend_batch_dims(tensor=param, shp=batch_shape_to_prepend)
            for name, param in dist_params.items()
        } if len(batch_shape_to_prepend) > 0 else dist_params
    ...
    def _flatten_batch_dims(self, x, batch_shp):
        ...
        if len(batch_shp) == 1:
            ...
            x_flat = torch.reshape(x, shape=(np.prod(
                tuple(batch_shp)),) + feature_shp)
        else:
    ...
    def _maybe_cat(self, x: (torch.Tensor, tuple, list, set)):
        if isinstance(x, (tuple, list, set)):
            if self.allow_cat_inputs:
                ...
                raise ValueError(f"got input of type {type(x)}, "
...
class LadderParametrisedConditionalDistribution(nn.Module):
    def __init__(self, stems: nn.ModuleList, dist_params: nn.ModuleList,
                 dist_cls,
                 n_feature_dims=1, allow_cat_inputs=False):
        super().__init__()
        self.n_feature_dims = n_feature_dims
        ...
        self.dist_cls = dist_cls
        self.allow_cat_inputs = allow_cat_inputs
        assert len(stems) == len(dist_params)
    ...
    def forward(self, x: (torch.Tensor, tuple, list, set),
        ...
        for idx_hierarchy, (stem, dist_params, dist_cls) in enumerate(
                zip(self.stems, self.dist_params, self.dist_cls)):
            ...
            dist_params = {
                name: prepend_batch_dims(tensor=param,
                                         shp=batch_shape_to_prepend)
                for name, param in dist_params.items()
            } if len(batch_shape_to_prepend) > 0 else dist_params
