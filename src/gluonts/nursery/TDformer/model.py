...
class LinearPositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        # Compute the positional encodings once in log space.
        pe = (
            torch.arange(0, max_len).float().unsqueeze(1).expand(-1, d_model)
            / max_len
        )
        pe.require_grad = False
...
class DataEmbedding_only(nn.Module):
    def __init__(
        self, c_in, d_model, embed_type="fixed", freq="h", dropout=0.1
    ):
        super(DataEmbedding_only, self).__init__()
...
class AttentionLayer(nn.Module):
    def __init__(
        self, attention, d_model, n_heads=8, d_keys=None, d_values=None
    ):
...
class EncoderLayer(nn.Module):
    def __init__(
        self, attention, d_model, d_ff=2048, dropout=0.1, activation="relu"
    ):
