...
class Exp_Main(Exp_Basic):
    ...
    def train(self):
        ...
        early_stopping = EarlyStopping(
            patience=self.args.patience, des=self.args.des, verbose=True
        )
        ...
        for epoch in range(1000):
            ...
            vali_loss, vali_mae = self.vali(
                vali_data, vali_loader, criterion, mae_criterion
            )
            test_loss, test_mae = self.vali(
                test_data, test_loader, criterion, mae_criterion
            )
            f = open("./log/" + self.args.des + ".txt", "a")
            f.write("\n")
            f.write(
                "epoch:{}, train_loss:{}, vali_loss:{}, test_mse:{}, test_mae:{}".format(
                    epoch + 1, train_loss, vali_loss, test_loss, test_mae
                )
            )
            f.close()

            print(
                "Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} Test MSE: {4:.7f} Test MAE: {5:.7f}".format(
                    epoch + 1,
                    train_steps,
                    train_loss,
                    vali_loss,
    ...
    def test(self, test=0):
        ...
        if test:
            ...
            self.model.load_state_dict(
                torch.load(
                    os.path.join(
                        "./checkpoints/" + self.args.des + "checkpoint.pth"
                    )
                )
            )
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                if self.args.output_attention:
                    ...
                    outputs, tr_ori, se_ori, tr_pred, se_pred = self.model(
                        batch_x, batch_x_mark, dec_inp, batch_y_mark
                    )
                ...
                if i % 1 == 0:
                    ...
                    if self.args.output_stl:
                        ...
                        visual_stl(
                            ...
                            se_pred,
                            self.args.seq_len,
                            os.path.join(folder_path, str(i) + "stl.pdf"),
                        )
