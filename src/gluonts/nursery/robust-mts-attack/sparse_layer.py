...
class SparseNet(nn.Module, ABC):  # accept x, return w for which E[|| w ||_0] <= m -- our sparse layer
    def __init__(self, context_length, target_dim, target_item, hidden_dim, m, max_norm, norm = True):
        ...
        self.m = m
        self.norm = norm
        self.gamma = nn.Parameter(-1.0 * torch.ones((1, self.target_dim)), requires_grad = True).to(device)
        ...
        self.dist = Normal(torch.FloatTensor([0.0]).to(device), torch.FloatTensor([1.0]).to(device))
        self.target_item = target_item
        self.max_norm = max_norm
        # self.use_mean = use_mean

    def _r(self):  # output is of size 1 by input_dim
        return (self.m / np.sqrt(self.target_dim)) * torch.exp(0.5 * self.gamma) / torch.sqrt(torch.sum(torch.exp(self.gamma)))
    ...
    def forward(self, x, n_sample=100):  # x is batch_size by input_dim
        ...
        eps = torch.empty(n_sample, mu.shape[0], self.input_dim).normal_(0, 1).to(device)
        w = mu + eps * std
        w = w.mean(0)
        ...
        # renorm output
        output = w.view(-1, self.context_length, self.target_dim) * (mask.float() * mask_target_item).reshape(-1, 1, self.target_dim).repeat(1, self.context_length, 1) # batch x context x target dim  
        output.clamp_(min=-self.max_norm, max=self.max_norm)
        return output.view(-1, self.context_length, self.target_dim)  # output size = batch_size * input_dim