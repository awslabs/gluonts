...
class SparseLayerAttack:
    ...
    def attack_batch(self, batch, true_future_target):
        ...
        for mode in self.params.modes:
            ...
            # generate target
            adv_target = generate_adv_target(mode, future_target, self.params.factor)

            # traverse over each sparsity
            for i, s in enumerate(self.params.sparsity):

                # Generate sparse attack using sparse layer
                sparse_layer = SparseNet(context_length=context_length, target_dim=dim, target_item=self.params.target_items[0], hidden_dim=40, m=s, max_norm=max_norm).cuda()
                optimizer = optim.Adam(sparse_layer.parameters(), lr=self.params.learning_rate)

                # iteration steps
                for _ in tqdm(range(self.params.n_iterations)):
                    optimizer.zero_grad()
                    sparse_delta = sparse_layer(batch['past_target_cdf'], n_sample=100).view(batch_size, context_length, dim)
                    perturbed_inputs = dict([(key, batch[key]) for key in self.input_names])
                    perturbed_inputs['past_target_cdf'] = batch['past_target_cdf'] * (1 + sparse_delta)
                    _, perturbed_mu = forward_model(self.model, perturbed_inputs)
                    loss = loss_function(perturbed_mu, adv_target)[:, self.params.attack_idx, self.params.target_items].mean()
                    loss.backward()
                    optimizer.step()
                ...
                with torch.no_grad():
                    ...
                    perturbation = sparse_layer(batch['past_target_cdf'], n_sample=100).view(batch_size, context_length, dim)   
                    perturbed_inputs = {key: batch[key] for key in self.input_names}
                    perturbed_inputs['past_target_cdf'] = perturbed_inputs['past_target_cdf'] * (1 + perturbation)

                    # get model outputs
                    _, perturbed_mu = forward_model(self.model, perturbed_inputs)
                    # compute loss
                    loss = loss_function(perturbed_mu[:, self.params.attack_idx, self.params.target_items], true_future_target[:, self.params.attack_idx, self.params.target_items]).mean()
                    if loss >= best_loss[i]:
                        best_loss[i] = loss
...
class AttackLoss(nn.Module):
    ...
    def forward(self, perturbation, mu, adv_target):
        ...
        distance_tensor = loss_function(adv_target, mu)[..., self.target_items].sum(-1) # batch x len(attack_idx)
        ...
        norm_tensor = loss_function(perturbation, zero).sum(-1) # batch x context_length
...
class AttackModule(nn.Module):
    ...
    def __init__(self, model, params, c, batch, input_names):
        ...
        self.attack_loss = AttackLoss(c, attack_idx=self.params.attack_idx, device=self.params.device, target_items=self.params.target_items)
        
        # Initialize perturbation: batch x context length x target dim
        self.perturbation = nn.Parameter(torch.zeros_like(batch['past_target_cdf']), requires_grad=True)
        self.perturbation.to(self.params.device)

    # Returns predicted mean and scale
    def forward(self, num_parallel_samples=100):
        perturbed_inputs = dict([(key, self.batch[key]) for key in self.input_names])
        perturbed_inputs['past_target_cdf'] = self.batch['past_target_cdf'] * (1 + self.perturbation)
...
class SparseAttackModule(nn.Module):
    ...
    def __init__(self, model, params, c, batch, input_names, attack_item):
        ...
        self.perturbation = nn.Parameter(torch.zeros_like(batch['past_target_cdf']), requires_grad=True)
        self.perturbation.to(self.params.device)
        self.mask = torch.zeros_like(self.perturbation, device=self.params.device)
        self.mask[..., attack_item] = 1

    # Returns predicted mean and scale
    def forward(self, num_parallel_samples=100):
        perturbed_inputs = dict([(key, self.batch[key]) for key in self.input_names])
        perturbed_inputs['past_target_cdf'] = self.batch['past_target_cdf'] * (1 + self.perturbation * self.mask)
        _, samples = forward_model(self.model, perturbed_inputs)
        return samples
...
class Attack:
    ...
    def attack_step(self, attack_module, optimizer, adv_target, ground_truth=None):
        ...
        # targeted loss
        _, _, _, _, _, loss = attack_module.attack_loss(attack_module.get_perturbation(), perturbed_mu, adv_target)

        # backward pass
        loss.backward()

        # renorm grad
        grad_norm = attack_module.get_grad().view(batch_size, -1).norm(p=2, dim=1)
    ...
    def attack_batch(self, batch, true_future_target):
        ...
        for mode in self.params.modes:
            ...
            adv_target = generate_adv_target(future_target=future_target,
                                                               mode=mode, factor=self.params.factor)
            ...
            with torch.no_grad():
                perturbed_mu = attack_module()
                loss = loss_function(true_future_target[:, attack_idx, target_items], perturbed_mu[:, attack_idx, target_items]).mean()
                if loss >= best_dense:
                    best_dense = loss
