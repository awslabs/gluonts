...
import os
from utils import Metrics, Params, calc_loss, load_pickle, change_device, PREDICTION_INPUT_NAMES, smoothed_inference
    ...
    with torch.no_grad():
        for i in tqdm(range(len(attack_data))):
            ...
            for s in params.sparsity:
                ...
                attacked_past_target = change_device(batch["past_target_cdf"], device) * (1 + perturbation_tensor) # perturbation: batch x context length x target dim
