...
from utils import AttackResults, Params, PREDICTION_INPUT_NAMES, change_device, ts_iter, requires_grad_
...
def main():
    ...
    true_future_targets = [tss[i].to_numpy()[-prediction_length:] for i in range(len(tss))]
    testset_idx = 0
    ...
    filename = "./attack_results_var/" + model_type + "_" + str(prediction_length) + "_" + args.dataset + "_sigma_" + str(noise_level) + "_" \
                + "rank" + str(predictor.prediction_net.distr_output.rank) + '_dim' + str(predictor.prediction_net.distr_output.dim)
    ...
    with open(filename, "wb") as outp:
        for batch in test_loader:
            ...
            future_target = np.array(true_future_targets[testset_idx: testset_idx + this_batch])
            best_perturbation = attack.attack_batch(batch,
