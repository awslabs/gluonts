...
class GMMModel(gluon.HybridBlock):
    ...
    def __init__(self, num_clusters, num_hidden, mu_=None, kR_=None, lr_mult=None):
        super().__init__()
        self.num_hidden = num_hidden

        with self.name_scope():
            self.mu_ = self.params.get(
                'mu_', shape=(num_clusters, num_hidden),
                lr_mult=lr_mult,
                init=None if mu_ is None else mx.init.Constant(mu_))

            self.kR_ = self.params.get(
                'kR_', shape=(num_clusters, num_hidden, num_hidden),
                lr_mult=lr_mult,
    ...
    def _get_Rx_(F, dx_, kR_):
        ...
        kR_expand_0 = F.broadcast_like(
            kR_.expand_dims(0), dx_,
            lhs_axes=(0,), rhs_axes=(0,))

        Rx_ = F.batch_dot(kR_expand_0,
                          dx_.expand_dims(-1)).squeeze(axis=-1)
        return Rx_
    ...
    def hybrid_forward(self, F, x, mu_, kR_):
        ...
        log_conditional = (- 0.5 * (Rx_**2).sum(axis=-1)
    ...
    def m_step(x, qz):
        ...
        sum_x2 = (qz[:,:,None,None] * (x[:,None,:,None] @ x[:,None,None,:])
                 ).sum(axis=0)
