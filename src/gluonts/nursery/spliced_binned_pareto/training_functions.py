...
def train_step_from_batch(ts_chunks: torch.Tensor, 
...
def eval_on_series(distr_tcn: DistributionalTCN, 
                   optimizer: torch.optim.Adam, 
    ...
    if is_train:
        ...
        unfold_layer = torch.nn.Unfold(kernel_size=(1,window_length), stride=stride)
        fold_layer = torch.nn.Fold(kernel_size=(1,window_length), stride=stride, 
                                   output_size=(1,series_tensor.shape[-1]))

        ts_windows = unfold_layer(series_tensor.unsqueeze(2)).transpose(1,2).transpose(0,1)
        ...
        if trying_mini_batches:
            ...
            for i in range(numb_mini_batches):
                ...
                loss_log.append(train_step_from_batch(ts_chunks, targets, distr_tcn, optimizer))
    ...
    for i in range(ts_len - context_length - lead_time - 1):
        ...
        if return_predictions:
            ...
            predictions['low_lower'].append(distr_output.icdf(torch.tensor(0.01)))
            predictions['up_upper'].append(distr_output.icdf(torch.tensor(0.99)))
...
def plot_prediction(val_ts_tensor: torch.Tensor, 
    ...
    for key in predictions.keys():
        try:
            predictions[key] = np.array(list(map(lambda x: x.detach().cpu().numpy(), predictions[key] ))).ravel()
        except:
            pass
    
    if fig is None:
        fig = plt.figure(figsize=(16,7))
    plt.plot(val_ts_tensor.cpu().flatten()[context_length+lead_time-1:][start:end], 'ko', markersize=2, label=r'$x_t$')
    plt.plot(predictions['up_upper'][start:end], 'r-', alpha=0.3, label=r'$z_{0.01}(t)|x_{t-1}$')
    plt.plot(predictions['upper'][start:end], 'y-', alpha=0.3, label=r'$z_{0.05}(t)|x_{t-1} \rightarrow \tau_t^{\rm{upper}}$')
    plt.fill_between(np.arange(start, end), predictions['lower'][start:end], predictions['upper'][start:end], facecolor='y', alpha=0.3, label=None)
    plt.plot(predictions['median'][start:end], 'g', label=r'$z_{0.5\,}(t)|x_{t-1}$')
    plt.plot(predictions['lower'][start:end], 'y-', alpha=0.3, label=r'$z_{0.95}(t)|x_{t-1} \rightarrow \tau_t^{\rm{lower}}$')
    plt.plot(predictions['low_lower'][start:end], 'r-', alpha=0.3, label=r'$z_{0.99}(t)|x_{t-1}$')
    
    plt.legend(loc='upper right')
    
    median = np.array(predictions['median'][start:end])
    true_val = np.array(val_ts_tensor.cpu().flatten()[context_length+lead_time-1:][start:end])
