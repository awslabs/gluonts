...
def add_spikes(ts: torch.Tensor, 
    ...
    indices_for_gp_spikes = np.random.choice(np.arange(len(ts)), replace=False, size=num_spikes)
...
def create_ds_iid(num_points: int, 
                  noise_mult: float=0.25):
...
def add_spikes_asymmetric(ts: torch.Tensor,
    ...
    idx = 0
    spikes = stats.genpareto(xi[idx]).rvs(half_num_spikes[idx]) * spike_direction[idx]
...
def create_ds_asymmetric(num_points: int, 
    ...
    idx = 0
    noise_lower = np.random.standard_t(t_dof[idx], num_points) * noise_mult[idx]
    noise_lower = np.minimum(0, noise_lower)

    idx = 1
    noise_upper = np.random.standard_t(t_dof[idx], num_points) * noise_mult[idx]
