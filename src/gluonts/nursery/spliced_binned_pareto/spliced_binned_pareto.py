...
class Binned(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        assert bins_lower_bound.shape.numel()==1, f'bins_lower_bound needs to have shape torch.Size([1])'
        assert bins_upper_bound.shape.numel()==1, f'bins_upper_bound needs to have shape torch.Size([1])'
    ...
    def log_binned_p(self, xx):
        ...
        if xx > self.bin_edges[-1]:
            one_hot_bin_indicator[-1] = 1.
        elif xx < self.bin_edges[0]:
            one_hot_bin_indicator[0] = 1.
        if not (one_hot_bin_indicator==1).sum()==1:
            print(f'Warning in log_p(self, xx): for xx={xx.item()}, one_hot_bin_indicator value_counts are {one_hot_bin_indicator.unique(return_counts=True)}')
    ...
    def cdf_binned_components(self, xx, idx=0, cum_density=torch.tensor([0.])):
        """
        ...
        bin_cdf_relative = torch.tensor([0.])
        if idx == 0:
            cum_density = torch.tensor([0.])
    ...
    def cdf_components(self, xx, idx=0, cum_density=torch.tensor([0.])):
        """
    ...
    def cdf(self, x):
        ...
        idx, cum_density, bin_cdf_relative = self.cdf_components(x[0], idx=0, cum_density=torch.tensor([0.]))
...
class SplicedBinnedPareto(Binned):
    ...
    def __init__(self, 
                 ...
                 validate_args=None):
        super().__init__(bins_lower_bound, bins_upper_bound, nbins, validate_args)
        
        assert percentile_gen_pareto>0 and percentile_gen_pareto<1, 'percentile_gen_pareto must be between (0,1)'
    ...
    def log_p(self, xx, for_training = True):
        ...
        logp_bins = self.log_binned_p(xx) + torch.log(1 - 2 * self.percentile_gen_pareto)
        logp = logp_bins 
        ...
        if xx > upper_percentile:
            ...
            logp_gen_pareto = self.upper_gen_pareto.log_prob(xx - upper_percentile) + torch.log(self.percentile_gen_pareto)
        ...
        # Log-prob given upper tail distribution    
            ...
            logp_gen_pareto = self.lower_gen_pareto.log_prob(lower_percentile - xx) + torch.log(self.percentile_gen_pareto)
            logp = logp_gen_pareto
            if for_training:
                logp += logp_bins
    ...
    def cdf_components(self, xx, idx=0, cum_density=torch.tensor([0.])):
        ...
        if xx < lower_percentile:
            ...
            cum_density = (1. - self.lower_gen_pareto.cdf(adjusted_xx)) * self.percentile_gen_pareto
        elif xx <= upper_percentile:
            idx, cum_density, bin_cdf_relative = self.cdf_binned_components(xx, idx, cum_density)
        else:
            adjusted_xx = xx - upper_percentile
            cum_density = (1.-self.percentile_gen_pareto) + self.upper_gen_pareto.cdf(adjusted_xx) * self.percentile_gen_pareto
        return idx, cum_density, bin_cdf_relative
    ...
    def inverse_cdf(self, value):
        ...
        Inverse cdf of a single percentile `value`
        """
        assert value>=0. and value<=1., 'percentile value must be between 0 and 1 inclusive'
        
        if value < self.percentile_gen_pareto:
            adjusted_percentile = 1 - (value / self.percentile_gen_pareto)
            icdf_value = self.inverse_binned_cdf(self.percentile_gen_pareto) - self.lower_gen_pareto.icdf(adjusted_percentile)
        elif value <= 1 - self.percentile_gen_pareto:
            icdf_value = self.inverse_binned_cdf(value)
        else:
            adjusted_percentile = (value - (1.-self.percentile_gen_pareto)) / self.percentile_gen_pareto
            icdf_value = self.upper_gen_pareto.icdf(adjusted_percentile) + self.inverse_binned_cdf(1 - self.percentile_gen_pareto)
