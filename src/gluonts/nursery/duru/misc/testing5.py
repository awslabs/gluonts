...
q = D.Independent(D.Normal(loc=q_loc, scale=q_scale), reinterpreted_batch_ndims=2)

p_loc = torch.randn(10, 5, 20)
p_scale = torch.clip(torch.randn(10, 5, 20), min=0.001)
p = D.Independent(D.Normal(loc=p_loc, scale=p_scale), reinterpreted_batch_ndims=2)

res1 = D.kl.kl_divergence(q, p)
res2 = gaussian_analytical_kl(q_loc, p_loc, torch.log(q_scale), torch.log(p_scale)).sum(
    dim=(1, 2)
)
