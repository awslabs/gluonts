...
class DomAdaptTrainer(Trainer):
    ...
    def _validate(self, *data):
        ...
        self.metrics.valid.src.fc_ND.update(self.model.src.fc_loss, self.model.src.denominator)
        self.metrics.valid.tgt.bc_loss.update(self.model.tgt.bc_loss)
        self.metrics.valid.tgt.fc_loss.update(self.model.tgt.fc_loss)
        self.metrics.valid.tgt.fc_ND.update(self.model.tgt.fc_loss, self.model.tgt.denominator)
    ...
    def from_configs(cls,
        ...
        meters = MeterDict(meterdicts={
            'train': MeterDict(
                ...
                meterdicts=dict(
                    src=MeterDict(meters=dict(
                        bc_loss=BatchAverageMeter(),
                        fc_loss=BatchAverageMeter(),
                    )),
                    tgt=MeterDict(meters=dict(
                        bc_loss=BatchAverageMeter(),
                        fc_loss=BatchAverageMeter(),
                    )),
                ),
            ),
            'valid': MeterDict(meterdicts=dict(
                src=MeterDict(meters=dict(
                    bc_loss=BatchAverageMeter(),
                    fc_loss=BatchAverageMeter(),
            ...
            'test': MeterDict(meterdicts=dict(
                src=MeterDict(meters=dict(
                    bc_loss=BatchAverageMeter(),
                    fc_loss=BatchAverageMeter(),
                    fc_ND=MeanDeviationMeter(),
...
class DomAdaptEvaluator(Evaluator):
    def _test(self, *data):
        ...
        self.metrics.test.src.fc_ND.update(self.model.src.fc_loss, self.model.src.denominator)
        self.metrics.test.tgt.bc_loss.update(self.model.tgt.bc_loss)
        self.metrics.test.tgt.fc_loss.update(self.model.tgt.fc_loss)
        self.metrics.test.tgt.fc_ND.update(self.model.tgt.fc_loss, self.model.tgt.denominator)
    ...
    def _predict(self, *data) -> np.ndarray:
        ...
        src_dummy = np.zeros(src_truth.shape[:-1]+src_preds.shape[-1:]) + np.nan
        src_preds = np.concatenate([src_dummy[:,:-src_preds.shape[1]], src_preds], axis=1)
        src_curve = np.concatenate([src_truth, src_preds], axis=-1)
        tgt_dummy = np.zeros(tgt_truth.shape[:-1]+tgt_preds.shape[-1:]) + np.nan
        tgt_preds = np.concatenate([tgt_dummy[:,:-tgt_preds.shape[1]], tgt_preds], axis=1)
...
class AdversarialDomAdaptTrainer(DomAdaptTrainer):
    ...
    def from_configs(cls,
        ...
        return cls(
            dataset, model, optimizer, None,
            metrics=meters, callbacks=callbacks, log_dir=log_dir,
            n_epochs=n_epochs, nb_epoch=nb_epoch, batch_size=batch_size, 
            cuda_device=cuda_device, debug=debug,
        )
...
class AdversarialDomAdaptEvaluator(DomAdaptEvaluator):
    ...
    def from_trainer(cls,
        ...
        return super(AdversarialDomAdaptEvaluator, cls).from_trainer(trainer, model_tag)