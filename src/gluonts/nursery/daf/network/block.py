...
class EncoderModule(nn.Module):
    def __init__(self,
        ...
        for d_s, wsize in zip(d_shape, window_size):
            self._shape_weights.append(nn.Parameter(Tensor(d_s, d_data+d_feats, wsize)))
        ...
        if self.tie_casts:
            ...
            self._extra_weight = nn.Parameter(Tensor(d_value, d_data+d_feats))
    ...
    def _compute_value(self, data: Tensor):
        ...
        extra_value = F.linear(data[:, max(self.window_size):], self._extra_weight, self._extra_bias)
