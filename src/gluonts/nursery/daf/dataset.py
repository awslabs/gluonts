...
from tslib.dataset import (
    TimeSeries, TimeSeriesCorpus,
    WindowsDataset, MetaDataset
)
from data import (
    SeasonalReader,
    FourierSignalReader, 
...
def _tensor_collator(batch: List[Tensor]):
    ...
    storage = elem.storage()._new_shared(len(batch) * max_length * elem[0].numel())
    ...
    if max(pad_size) == 0:
        ...
        tensor = pt.stack([
            pt.cat([sample.new_zeros(pad, *sample.shape[1:]), sample], dim=0)
...
def _pair_variable_length_collator(batch: List[Tuple]):
    (
        src_data, tgt_data, 
        src_feats, tgt_feats,
        src_nan_mask, tgt_nan_mask,
    ) = zip(*batch)
    if src_data[0] is None:
        src_input = (None, None, None, None)
    else:
        src_input = _variable_length_collator(list(zip(src_data, 
                                                       src_feats,
                                                       src_nan_mask)))
    if tgt_data[0] is None:
        tgt_input = (None, None, None, None)
    else:
        tgt_input = _variable_length_collator(list(zip(tgt_data,
                                                       tgt_feats,
                                                       tgt_nan_mask)))
...
class DomAdaptDataset(TorchDataset):
    def __init__(self,
        ...
        self._source_size = 1 if self.source_dataset is None else len(self.source_dataset)
        self._target_size = 1 if self.target_dataset is None else len(self.target_dataset)
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        tgt_index = index
        src = None if self.source_dataset is None else self.source_dataset[src_index]
        tgt = None if self.target_dataset is None else self.target_dataset[tgt_index]
        if src is None:
    ...
    def from_domains(cls,
        ...
        if (source_dataset is not None) and (source_dataset.collate_fn is _variable_length_collator):
            kwargs.setdefault('collate_fn', _pair_variable_length_collator)
        if (target_dataset is not None) and (target_dataset.collate_fn is _variable_length_collator):
            kwargs.setdefault('collate_fn', _pair_variable_length_collator)
...
class SeasonalDataset(WindowsDataset, SeasonalReader):
    ...
    def create_dataset(cls,
        ...
        train_corpus = cls.load_data(n_instances_train, n_samples, *freqs, **kwargs)
        valid_corpus = cls.load_data(n_instances_test, n_samples, *freqs, **kwargs)
        test_corpus = cls.load_data(n_instances_test, n_samples, *freqs, **kwargs)
        dataset = MetaDataset(
...
class BenchmarkDataset(WindowsDataset, GluonTSJsonReader):
    def __getitem__(self, index: int) -> Tuple[Tensor, Optional[Tensor], Optional[Tensor]]:
        ...
        if len(feat_names) > 0:
            feats = pt.tensor(
                np.concatenate(
                    [window.revealed_numerical_features[k] for k in feat_names],
                    axis=1,
                ), 
                dtype=pt.float,
            )
    ...
    def create_dataset(cls,
        ...
        if validation:
            train_corpus, valid_corpus = train_corpus.split_from_end(eval_size, 0, raise_error=False)
        else:
            valid_corpus = None
        if train_size is not None:
            _, train_corpus = train_corpus.split_from_end(train_size, 0, raise_error=False)
