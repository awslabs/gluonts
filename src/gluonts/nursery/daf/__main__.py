...
from .estimator import (
    AttentionEstimator, AdversarialEstimator,
    DomAdaptEstimator, AdversarialDomAdaptEstimator,
)
from .engine import (
    AttentionTrainer, 
    AttentionEvaluator,
...
def parse_configs(cmd=None):
    ...
    model_configs.add_argument('--not_scale_input', action='store_false', dest='normalize_input')
    model_configs.add_argument('--not_scale_output', action='store_false', dest='rescale_output')
    model_configs.add_argument('--last_layer_loss_only', action='store_false', dest='layerwise_loss')
...
with Path(__file__).parents[1].joinpath(f'dumps/{exp_name}/defaults.json').open('r') as f:
