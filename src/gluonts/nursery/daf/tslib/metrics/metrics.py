...
def tweedie_loss(p: float):
    def tweedie_ll(preds: Tensor, target: Tensor):
        return -target*pt.pow(preds, 1-p)/(1-p) + pt.pow(preds, 2-p)/(2-p)
    return tweedie_ll