...
class BatchAverageMeter(NumericalAverageMeter):
    ...
    '''
    def __init__(self, 
            min_mode: Optional[bool] = True, 
            avg_in_batch: bool = True):
...
class MeanDeviationMeter(Meter):
    ...
    def update(self, 
        ...
        if deviation.size() != base.size():
            raise ValueError('`deviation` and `base` should matchh in dimensions')
    ...
    def value(self) -> float:
        if self._cum_base == 0.0:
            warnings.warn('Cumulative base is 0. '
                          'Inf is returned.')
            return float('inf')
        else:
...
class RootMeanSquareDeviationMeter(MeanDeviationMeter):
    ...
    def update(self, 
        ...
        self.cache = deviation.pow(2).sum().pow(0.5).div(base.pow(2).sum().pow(0.5)).item()
