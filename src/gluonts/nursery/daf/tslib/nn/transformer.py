...
from .attention import SelfAttention, GroupSelfAttention, CompositeAttention, InterAttention
...
class TransformerEncoderBlock(nn.Module):
    def __init__(self,
        ...
        if n_groups < 1:
            raise ValueError(f'n_groups must be a positive integer, but {n_groups} is given')
        elif n_groups == 1:
            if d_qk != d_value:
                warning.warn("When n_groups=1, `d_qk` and `d_value` is suggested to be equal, "
            ...
            Attention = CompositeAttention if composite_attention else GroupSelfAttention
    ...
    def forward(self,
        ...
        if not self.shape_qk:
            ...
            if shape is None:
                raise ValueError('if shape attention is enabled, '
                                 'a shape tensor must be provided.')
            if not self.composite_attention:
                x = self.attention(x, shape, mask=mask)
            else:
                if estimate is None:
                    raise ValueError('if composite attention is enabled, '
