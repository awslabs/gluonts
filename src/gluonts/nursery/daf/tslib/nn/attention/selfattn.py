...
class SelfAttention(Attention):
    ...
    def _apply_mask(self, 
                    score: Tensor,
                    key_mask: Optional[BoolTensor]) -> Tensor:
        if not self.bidirectional:
            unid_mask = score.new_ones(*score.shape[-2:], dtype=pt.bool).triu(diagonal=1)
    ...
    def _compute_attn_score(self,
        ...
        if self.apply_rel_dist is not None:
            ...
            idx = idx.view(1,1,klen,klen).expand(k.size(0),k.size(1),-1,-1)
    ...
    *NOTE*: d_qk, d_hidden, n_head must be divisible by n_groups
    ...
    def __init__(self,
        ...
        try:
            ...
            raise ValueError('d_qk, d_hidden, n_head must be divisible by n_groups')
        if d_qk // n_groups > d_value:
            raise ValueError('dimension of each group should not exceed d_value')
        self.d_qk = d_qk
    ...
    def _reset_parameters(self):
        ...
        self._qk_proj_weight = nn.Parameter(Tensor(2*self.d_hidden, self.d_qk//self.n_groups, 1))
...
class CompositeAttention(GroupSelfAttention):
    ...
    def _compute_qkv(self,
            value: Tensor,
            shape: Tensor, 
            estimate: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
