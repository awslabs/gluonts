...
class LearnablePositionalEmbedding(nn.Module):
    def __init__(self, 
        ...
        assert reduce(mul, sub_shape, 1) == max_len, f'decomposed shape {sub_shape} inconsistent with max length {max_len}'
        assert reduce(add, d_sub_embeds, 0) == d_embed, f'decomposed embed dim {d_sub_embeds} inconsistent with overall embed dim {d_embed}'
        self.d_embed = d_embed
    ...
    def forward(self, pos_seq: LongTensor):
        ...
        if pt.any(x > 0):
            ...
            raise ValueError(f'Given positional index at {exceed_idx} exceeds the max sequence length {self.max_len}')
