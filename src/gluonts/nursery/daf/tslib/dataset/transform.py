...
def add_datetime_features(
        ...
        normalized:         bool=True) -> TimeSeries:
    ...
    if add_month_of_year:
        ts.add_features('month_of_year', _month_of_year(time_index), data_type, feature_type)
    if add_week_of_year:
        ts.add_features('week_of_year', _week_of_year(time_index), data_type, feature_type)
    if add_day_of_year:
        ts.add_features('day_of_year', _day_of_year(time_index), data_type, feature_type)
    if add_day_of_month:
        ts.add_features('day_of_month', _day_of_month(time_index), data_type, feature_type)
    if add_day_of_week:
        ts.add_features('day_of_week', _day_of_week(time_index), data_type, feature_type)
    if add_hour_of_day:
        ts.add_features('hour_of_day', _hour_of_day(time_index), data_type, feature_type)
    if add_minute_of_hour:
        ts.add_features('minute_of_hour', _minute_of_hour(time_index), data_type, feature_type)
    return ts



def add_duration_feature(ts: TimeSeries, 
    ...
    ts.add_features('age', age.reshape(-1,1), data_type='numerical', feature_type='revealed')
    return ts