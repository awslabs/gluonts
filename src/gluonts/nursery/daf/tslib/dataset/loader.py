...
def copy_to_gpu(data, cuda_device: int, non_blocking: bool):
    ...
    if isinstance(data, pt.Tensor):
        ...
        return {key: copy_to_gpu(val, cuda_device, non_blocking) for key, val in data.items()}
    elif isinstance(data, container_abcs.Sequence):
        return [copy_to_gpu(t, cuda_device, non_blocking) for t in data]
    elif data is None:
        return None
    else:
        raise ValueError(f'expected tensor, sequence or dictionary of tensors, received {type(data).__name__}')
...
class MetaDataset(NamedTuple):
    ...
    def train_loader(self,
        ...
        return self._data_loader(self.train_set, 
            batch_size, shuffle, cuda_device, True, n_workers, n_batches,
        )
    ...
    def valid_loader(self,
        ...
        return self._data_loader(dataset, 
            batch_size, False, cuda_device, False, n_workers, None,
        )
    ...
    def test_loader(self,
        ...
        return self._data_loader(self.test_set, 
            batch_size, False, cuda_device, False, n_workers, None,
        )
