...
class ZScoreNormalizer(object):
    ...
    def forward_pre_hook(self, module, input):
        ...
        if (length is None) and (nan_mask is None):
            ...
            if nan_mask is None:
                ...
                if length is None:
                    count = nan_mask.logical_not().float().sum(dim=1, keepdim=True)
                else:
                    count = length.unsqueeze(dim=1).float() - nan_mask.float().sum(dim=1, keepdim=True)
    ...
    def forward_hook(self, module, input, output):
        ...
        rescaled_forecast = forecast * scale.unsqueeze(dim=2) + offset.unsqueeze(dim=2)
...
class LossFunction(object):
    ...
    def _aggregate_loss(self, loss: Tensor, loss_mask: Optional[BoolTensor]) -> Tensor:
        ...
        if loss_mask is None:
            ...
            loss = loss.masked_fill(loss_mask.view(*loss_mask.shape, 1, 1), 0.0)
            loss = loss.view(batch_size, -1).sum(dim=1)
    ...
    def loss_hook(self, module, input, output):
        ...
        module.denominator = self._aggregate_loss(target.unsqueeze(dim=2), fc_mask)
