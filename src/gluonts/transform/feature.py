...
def target_transformation_length(target: np.array, pred_length: int) -> int:
    return target.shape[-1] + pred_length
...
class AddObservedValuesIndicator(MapTransformation):
    """
...
class AddConstFeature(MapTransformation):
    ...
    pred_length
        Prediction length (this is necessary since features have to be available
        in the future)
    const
    ...
    def map_transform(self, data: DataEntry) -> DataEntry:
        length = target_transformation_length(
            data[self.target_field], self.pred_length
        )
...
class AddAggregateLags(MapTransformation):
    ...
    def map_transform(self, data: DataEntry) -> DataEntry:
        ...
        assert data[self.feature_name].shape == (
            len(self.valid_lags),
            len(data[self.target_field]) + self.pred_length,
        )
