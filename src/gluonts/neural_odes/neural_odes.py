...
class NeuralLV():
    ...
    def solve_discrete_lv(self, mat1, mat2=None, is_full_matrix=True):
        ...
        return nd.concat(*p, dim=0).reshape(self.num_time_steps, self.num_time_series).T
...
class LowRankVectorEmbedding(gluon.HybridBlock):
    ...
    def __init__(self, neural_lv, **kwargs):
        ...
        self.feat_static_cat = nd.arange(self.num_time_series, ctx=self.ctx, dtype=self.dtype)
...
def lv_plot_ts(p, p_approx, max_num_plots=10, num_rows=2, fig_size_width=10): # plots all time series at corresponding time point
    ...
    for ts_idx in range(num_plots):
        ...
        plt.plot(t, p[ts_idx, :].asnumpy(), \
                 t, p_approx[ts_idx, :].asnumpy(), 'r--')
...
def compute_low_rank_product(B, C):
    return nd.dot(B, C, transpose_a = True) if C is not None \
...
def generate_data(num_ts, ctx=mx.gpu(), dtype='float64', seed=100): # num_ts = d
    np.random.seed(seed)
    ...
    return nd.array(r, ctx=ctx, dtype=dtype), nd.array(k, ctx=ctx, dtype=dtype), \
           nd.array(p0, ctx=ctx, dtype=dtype), nd.array(A, ctx=ctx, dtype=dtype)
