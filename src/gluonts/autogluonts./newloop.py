...
def newloop(
        epoch_no, estimator, net,trainer,inputs,test_data,metric,is_training: bool = True,
):
 #   print('check 0')
    tic = time.time()
 ...
 #   print('check 1 test')
    ...
    with mx.autograd.record():
      ...
      #  print('check 2.2')
        ...
        # when having multiple outputs, the forward returns a list in the case of hybrid and a        # tuple otherwise
   ...
   num_samples=100,)  # number of sample paths we want for evaluation
    ...
    result = call(iter(tss), iter(forecasts), metric,len(test_data))
    return result
